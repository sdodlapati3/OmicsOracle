...
class APIClient:
    ...
    def get_component_status(self, health_data):
        ...
        components.append({
            ...
            "message": "Pipeline is initialized and ready" if pipeline_status == "healthy" else "Pipeline not available"
        })
        ...
        components.append({
            "name": "GEO Client",
            "status": geo_status,
            "message": "GEO client is initialized" if geo_status == "healthy" else "GEO client not available"
        })
        ...
        components.append({
            ...
            "message": "Summarizer is initialized" if summarizer_status == "healthy" else "Summarizer not available"
        })
        ...
        components.append({
            "name": "NCBI Email",
            "status": email_status,
            "message": f"Email configured: {ncbi_email}" if email_status == "healthy" else "NCBI email not configured"
        })
        ...
        components.append({
            ...
            "message": f"Email configured: {entrez_email}" if entrez_status == "healthy" else "Bio.Entrez.email not configured"
        })
    ...
    try:
        while True:
            ...
            try:
                ...
                if message.get("action") == "refresh_status":
                ...
                elif message.get("action") in ["run_pipeline_diagnostics", "run_geo_test", "run_api_test"]:
                    # Run a test script
                    ...
                    for test_type in ["pipeline_diagnostics", "geo_test", "api_test"]:
                        await websocket.send_json({
                            "type": "log",
