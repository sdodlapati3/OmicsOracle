...
class AdvancedSearchValidator:
            ...
            try:
                ...
                if len(ranked_results) != len(test_case["results"]):
                    case_result["notes"].append(
                        "Result count changed after ranking"
                    )
                elif not all("semantic_score" in r for r in ranked_results):
                    case_result["notes"].append(
                        "Not all results have semantic scores"
                    )
                else:
                    # Check if results are sorted by semantic score
                    scores = [
                        r.get("semantic_score", 0) for r in ranked_results
                    ]
                    if scores != sorted(scores, reverse=True):
                        case_result["notes"].append(
                            "Results not properly sorted by semantic score"
                        )
                    else:
                        case_result["success"] = True
                        case_result["notes"].append(
                            f"Scores range: {min(scores):.2f} to {max(scores):.2f}"
                        )
    ...
    def validate_result_clustering(self) -> Dict[str, Any]:
        ...
        for test_case in self.test_cases:
            ...
            try:
                # Apply clustering
                clustered_results = self.enhancer.cluster_results(
                    test_case["results"].copy()
                )
                ...
                if "clusters" not in clustered_results:
                    ...
                    case_result["notes"].append(
                        "Clusters not returned as a list"
                    )
                elif (
                    len(test_case["results"]) >= 3
                    and len(clustered_results["clusters"]) == 0
                ):
                    case_result["notes"].append(
                        "No clusters found for a query with diverse results"
                    )
                else:
                    case_result["success"] = True
                    case_result["notes"].append(
                        f"Found {len(clustered_results['clusters'])} clusters"
                    )

                    # Additional checks for the diverse results test case
                    if (
                        test_case["name"] == "diverse_results"
                        and len(clustered_results["clusters"]) < 2
                    ):
                        case_result["notes"].append(
                            "Expected multiple clusters for diverse results"
                        )
                        case_result["success"] = False
    ...
    def validate_query_reformulation(self) -> Dict[str, Any]:
        ...
        for test_case in self.test_cases:
            ...
            try:
                ...
                reformulations = self.enhancer.generate_query_reformulations(
                    test_case["query"]
                )
                ...
                if not isinstance(reformulations, list):
                    case_result["notes"].append(
                        "Reformulations not returned as a list"
                    )
                elif (
                    test_case["name"] == "missing_data_type"
                    and len(reformulations) == 0
                ):
                    case_result["notes"].append(
                        "No reformulations suggested for query missing data type"
                    )
                else:
                    case_result["success"] = True
                    case_result["notes"].append(
                        f"Generated {len(reformulations)} reformulations"
                    )
                    ...
                    for ref in reformulations:
                        if not all(
                            k in ref
                            for k in ["query", "explanation", "confidence"]
                        ):
                            case_result["notes"].append(
                                "Reformulation missing required fields"
                            )
                            case_result["success"] = False
                            break
        ...
        for test_case in self.test_cases:
            ...
            try:
                ...
                if "results" not in enhanced_results:
                    ...
                    case_result["notes"].append(
                        "No enhancements listed in output"
                    )
                else:
                    case_result["success"] = True
                    ...
                    if "semantic_ranking" in applied:
                        if not all(
                            ...
                            case_result["notes"].append(
                                "Semantic ranking claimed but scores missing"
                            )
                            case_result["success"] = False

                    if (
                        "clustering" in applied
                        and "clusters" not in enhanced_results
                    ):
                        case_result["notes"].append(
                            "Clustering claimed but clusters missing"
                        )
                        case_result["success"] = False

                    if (
                        "query_reformulations" in applied
                        and "query_reformulations" not in enhanced_results
                    ):
                        case_result["notes"].append(
                            "Query reformulations claimed but missing"
                        )
                        case_result["success"] = False
    ...
    def run_all_validations(self) -> Dict[str, Any]:
        ...
        all_results["validations"][
            "semantic_ranking"
        ] = self.validate_semantic_ranking()
        all_results["validations"][
            "result_clustering"
        ] = self.validate_result_clustering()
        all_results["validations"][
            "query_reformulation"
        ] = self.validate_query_reformulation()
        all_results["validations"][
            "full_pipeline"
        ] = self.validate_full_enhancement_pipeline()

        # Calculate overall results
        total_passed = sum(
            v["passed"] for v in all_results["validations"].values()
        )
        total_tests = sum(
            v["passed"] + v["failed"]
    ...
    def save_validation_results(
        ...
        json_path = (
            self.results_dir / f"advanced_search_validation_{timestamp}.json"
        )
        ...
        md_path = (
            self.results_dir
            / f"advanced_search_validation_report_{timestamp}.md"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OmicsOracle Advanced Search Features Validation"
    )
    parser.add_argument(
        "--feature",
        choices=[
    ...
    if args.feature == "semantic_ranking":
        results = {
            "timestamp": datetime.now().isoformat(),
            "validations": {
                "semantic_ranking": validator.validate_semantic_ranking()
            },
        ...
        results = {
            "timestamp": datetime.now().isoformat(),
            "validations": {
                "result_clustering": validator.validate_result_clustering()
            },
        ...
        results = {
            "timestamp": datetime.now().isoformat(),
            "validations": {
                "query_reformulation": validator.validate_query_reformulation()
            },
        ...
        results = {
            "timestamp": datetime.now().isoformat(),
            "validations": {
                "full_pipeline": validator.validate_full_enhancement_pipeline()
            },
