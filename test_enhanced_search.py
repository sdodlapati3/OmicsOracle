...
async def test_search_with_enhanced_handler():
    ...
    try:
        for query in test_queries:
            ...
            try:
                # Standard approach
                standard_result = await pipeline.process_query(
                    query, max_results=5
                )
                standard_count = (
                    len(standard_result.geo_ids)
                    if hasattr(standard_result, "geo_ids")
                    else 0
                )
                logger.info(
                    f"Standard search results: {standard_count} GEO IDs"
                )
            ...
            try:
                ...
                geo_ids, metadata = await perform_multi_strategy_search(
                    pipeline, query, max_results=5
                )
                enhanced_count = len(geo_ids)
                logger.info(
                    f"Enhanced search results: {enhanced_count} GEO IDs"
                )
                logger.info(
                    f"Search strategy: {metadata.get('search_strategy')}"
                )
    ...
    try:
        ...
        if pipeline:
            ...
            if hasattr(pipeline, "geo_client"):
                if (
                    hasattr(pipeline.geo_client, "session")
                    and not pipeline.geo_client.session.closed
                ):
            ...
            if hasattr(pipeline, "http_client") and hasattr(
                pipeline.http_client, "session"
            ):
