...
from src.omics_oracle.application.use_cases.search_datasets import (
...
from src.omics_oracle.domain.repositories.search_repository import (
    SearchRepository,
)
...
class MockSearchRepository(SearchRepository):
    ...
    async def search(self, query: SearchQuery):
        ...
        if query.organisms:
            filtered_datasets = [
                d for d in filtered_datasets if d.organism in query.organisms
            ]
...
async def test_clean_architecture_integration():
    ...
    for i, dataset_dto in enumerate(response.datasets):
        ...
        print(
            f"     Organism: {dataset_dto.organism}, Samples: {dataset_dto.samples_count}"
        )
    ...
    print(
        f"   - Average Relevance: {flow_response.average_relevance_score:.2f}"
    )
