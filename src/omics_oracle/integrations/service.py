...
class IntegrationService:
    ...
    async def enrich_geo_dataset(
        ...
        if include_papers:
            try:
                async with PubMedIntegration(
                    email=self.pubmed_email, api_key=self.pubmed_api_key
                ) as pubmed:
                    ...
                    logger.info(
                        f"Found {len(papers)} related papers for {geo_data.get('accession')}"
                    )
    ...
    async def batch_enrich_datasets(
        ...
        for dataset in geo_datasets:
            try:
                enriched = await self.enrich_geo_dataset(
                    dataset, include_papers, max_papers
                )
                enriched_datasets.append(enriched)

            except Exception as e:
                logger.error(
                    f"Failed to enrich dataset {dataset.get('accession', 'unknown')}: {e}"
                )
                # Add the original dataset with empty enrichment
                dataset["related_papers"] = []
                dataset[
                    "citation_info"
                ] = self.citation_manager.format_geo_reference(dataset)
    ...
    def export_citations(
        ...
        """
        return self.citation_manager.export_references(
