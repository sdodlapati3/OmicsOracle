...
async def advanced_search_v2(
    query: str = Query(
        ..., description="Advanced search query with semantic understanding"
    ),
    max_results: int = Query(
        20, ge=1, le=200, description="Maximum number of results"
    ),
    ...
    date_range: Optional[str] = Query(
        None, description="Filter by date range (YYYY-MM-DD:YYYY-MM-DD)"
    ),
    include_metadata: bool = Query(
        True, description="Include comprehensive metadata"
    ),
    enable_caching: bool = Query(
        True, description="Enable multi-level caching"
    ),
    ...
    try:
        ...
        if date_range:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid date range format"
                )
        ...
        if realtime_updates:
            ...
            asyncio.create_task(
                realtime_service.track_search_progress(
                    search_id, search_request
                )
            )
    ...
    try:
        ...
        stats = {
            ...
            "hierarchy_stats": {
                ...
                "promotion_count": getattr(
                    cache_hierarchy, "promotion_count", 0
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Cache stats unavailable: {str(e)}"
        )
...
async def get_service_registry_v2(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Service registry unavailable: {str(e)}"
        )
