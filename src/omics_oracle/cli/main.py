...
@click.option(
    "--include-sra", is_flag=True, help="Include SRA metadata in results"
)
@click.option(
    "--format",
    "-f",
...
def info(ctx, geo_id: str, include_sra: bool, output_format: str):
    ...
    async def run_info():
        ...
        try:
            ...
            metadata = await oracle.geo_client.get_geo_metadata(
...
@click.option(
    "--include-sra", is_flag=True, help="Include SRA data if available"
)
@click.pass_context
def download(
    ctx, geo_id: str, output_dir: str, download_format: str, include_sra: bool
):
...
def analyze(ctx, geo_id: str, output: str, output_format: str):
    ...
    async def run_analyze():
        ...
        try:
            ...
            entities = oracle.biomedical_ner.extract_biomedical_entities(
                combined_text
            )
...
@click.option(
    "--max-results", "-m", default=10, help="Maximum results per query"
)
@click.option(
    "--format",
    "-f",
        ...
        try:
            ...
            for i, query in enumerate(queries, 1):
                ...
                try:
                    ...
                    click.echo(
                        f"OK Query {i} completed: "
                        f"{len(result.metadata)} results"
                    )
...
@cli.command()
@click.option(
    "--max-results", "-m", default=10, help="Default maximum results per query"
)
@click.pass_context
            ...
            while True:
                try:
                    ...
                    if query.lower() in ["quit", "exit", "q"]:
                        ...
                        click.echo(
                            "  search <query> - Search for biological data"
                        )
    ...
    if result.entities:
        ...
        for entity_type, entities in result.entities.items():
            if entities:
                ...
                lines.append(
                    f"  {entity_type.title()}: {', '.join(entity_texts)}"
                )
        lines.append("")
