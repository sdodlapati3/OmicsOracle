...
class OmicsOracle:
    ...
    async def process_query(
        ...
        result.add_step("initialization", {"query": query, "query_id": query_id})
        ...
        try:
            ...
            raise OmicsOracleException(f"Pipeline processing failed: {str(e)}") from e
    ...
    async def _parse_query(self, result: QueryResult) -> None:
        ...
        intent_result = self.nlp_interpreter.classify_intent(result.original_query)
        ...
        entities = self.biomedical_ner.extract_biomedical_entities(result.original_query)
        ...
        if expanded_terms:
            result.expanded_query = " ".join([result.original_query] + list(expanded_terms)[:10])
        else:
    ...
    async def _search_geo_data(self, result: QueryResult, max_results: int) -> None:
        ...
        try:
            ...
            geo_ids = await self.geo_client.search_geo_series(search_query, max_results=max_results)
    ...
    async def _process_results(self, result: QueryResult, include_sra: bool) -> None:
        ...
        for geo_id in result.geo_ids[:20]:  # Limit to prevent overwhelming
            try:
                metadata = await self.geo_client.get_geo_metadata(geo_id, include_sra=include_sra)
                if metadata:
                    # Enhance metadata with relevance scoring
                    enhanced_metadata = await self._enhance_metadata(metadata, result)
                    metadata_list.append(enhanced_metadata)

            except Exception as e:
                logger.warning("Failed to get metadata for %s: %s", geo_id, str(e))
                continue
    ...
    async def _enhance_metadata(self, metadata: Dict[str, Any], result: QueryResult) -> Dict[str, Any]:
        ...
        text_content = (metadata.get("title", "") + " " + metadata.get("summary", "")).lower()
        ...
        for entity_type, entity_list in result.entities.items():
            for entity in entity_list:
                ...
                if entity_text in text_content:
                    relevance_score += 1.0
                    matched_entities.append({"type": entity_type, "text": entity_text})
    ...
    async def _format_results(self, result: QueryResult, result_format: ResultFormat) -> None:
        """Format results according to specified format."""
        logger.debug("Formatting results: %s", result.query_id)

        # Sort results by relevance score
        if result.metadata:
            result.metadata.sort(key=lambda x: x.get("relevance_score", 0), reverse=True)
    ...
    async def cleanup_completed_queries(self, max_age_hours: int = 24) -> int:
        ...
        for query_id, result in list(self._active_queries.items()):
            if result.end_time:
                age_hours = (current_time - result.end_time).total_seconds() / 3600
        ...
        try:
            ...
            for i, metadata in enumerate(top_results):
                try:
                    ...
                    dataset_id = (
                        metadata.get("accession") or metadata.get("id") or metadata.get("geo_id") or "unknown"
                    )
            ...
            if len(result.metadata) > 0:
                ...
                dataset_id = (
                    first_result.get("accession")
                    or first_result.get("id")
                    or first_result.get("geo_id")
                    or "unknown"
                )
