...
class ResearchContext:
    ...
    research_domains: List[ResearchDomain] = field(
        default_factory=lambda: [ResearchDomain.GENERAL_GENOMICS]
    )
            ...
            query_boost = (
                len(
                    [
                        ...
                        if any(
                            term in q.lower()
                            for term in ["cancer", "brain", "methylation"]
                        )
                    ]
                )
                * 5
            ...
            timeline_data.append(
                {
                    ...
                    "domains": [
                        domain.value for domain in context.research_domains
                    ],
                }
            )
        ...
        return {
            ...
            "metadata": {
                ...
                "total_publications": sum(
                    item["publications"] for item in timeline_data
                ),
                "relevant_publications": sum(
                    item["publications"]
                    for item in timeline_data
                    if item["relevant_to_user"]
                ),
                "domains": [
                    domain.value for domain in context.research_domains
                ],
...
class DatasetAvailabilityMatrixWidget(ResearchWidget):
    ...
    def get_visualization_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tooltip": {
                "format": "{organism} + {technique}: {value} datasets available"
            },
        }
...
class DashboardManager:
    ...
    def get_dashboard_config(self, context: ResearchContext) -> Dict[str, Any]:
        ...
        primary_domain = (
            context.research_domains[0]
            if context.research_domains
            else ResearchDomain.GENERAL_GENOMICS
        )
        config = self.domain_configs.get(
            primary_domain, self.domain_configs[ResearchDomain.GENERAL_GENOMICS]
        )
    ...
    def get_widget_data(
        self, widget_type: WidgetType, context: ResearchContext
    ) -> Dict[str, Any]:
...
async def get_widget_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating widget data: {str(e)}"
        )
...
async def get_research_dashboard():
    ...
    dashboard_file = (
        Path(__file__).parent / "static" / "research_dashboard.html"
    )
