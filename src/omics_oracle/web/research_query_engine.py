...
class ResearchQuery(BaseModel):
    ...
    response_format: str = (
        "comprehensive"  # comprehensive, summary, bullet_points
    )
...
class ResearchQueryEngine:
    ...
    async def _handle_research_gaps_query(
        self, query: ResearchQuery, entities: List[str]
    ) -> QueryResponse:
    ...
    async def _handle_methodology_query(
        ...
        research_goals = (
            [f"Study {entity}" for entity in entities]
            if entities
            else ["General genomics research"]
        )
        ...
        for i, rec in enumerate(recommendations[:3], 1):
            answer += (
                f"{i}. **{rec.technique}** (Confidence: {rec.confidence:.1%})\n"
            )
    ...
    async def _handle_comparison_query(
        ...
        for entity in entities[:3]:
            if entity in comparison_data:
                ...
                answer += (
                    f"- Technical Complexity: {data['complexity']:.1%}\n\n"
                )

        answer += "Recommendation: Choose based on your specific research requirements and budget constraints."
    ...
    async def _handle_data_summary_query(
        self, query: ResearchQuery, entities: List[str]
    ) -> QueryResponse:
        ...
        if (
            entities
            and entities[0] in research_intelligence.entity_knowledge_base
        ):
            entity_data = research_intelligence.entity_knowledge_base[
                entities[0]
            ]
    ...
    async def _handle_trends_query(
        self, query: ResearchQuery, entities: List[str]
    ) -> QueryResponse:
    ...
    async def _personalize_response(
        self, response: QueryResponse, user_id: str
    ) -> QueryResponse:
...
query_router = APIRouter(
    prefix="/api/research/query", tags=["research-queries"]
)
