...
class AnalyzeRequest(BaseModel):
    ...
    include_entity_linking: bool = Field(
        default=True, description="Include entity linking"
    )
    output_format: OutputFormat = Field(
        default=OutputFormat.JSON, description="Output format"
    )
...
class BatchRequest(BaseModel):
    ...
    max_results: int = Field(
        default=10, ge=1, le=100, description="Maximum results per query"
    )
    output_format: OutputFormat = Field(
        default=OutputFormat.JSON, description="Output format"
    )
...
class DatasetMetadata(BaseModel):
    ...
    sra_info: Optional[Dict[str, Any]] = Field(
        None, description="SRA information"
    )
...
class BatchResult(BaseModel):
    ...
    completed_queries: int = Field(
        default=0, description="Number of completed queries"
    )
    failed_queries: int = Field(
        default=0, description="Number of failed queries"
    )
    results: List[SearchResult] = Field(
        default=[], description="Individual query results"
    )
    status: QueryStatus = Field(..., description="Overall batch status")
    started_at: datetime = Field(
        default_factory=datetime.utcnow, description="Batch start time"
    )
    completed_at: Optional[datetime] = Field(
        None, description="Batch completion time"
    )
...
class StatusResponse(BaseModel):
    ...
    active_queries: int = Field(
        default=0, description="Number of active queries"
    )
