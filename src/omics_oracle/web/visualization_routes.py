...
class SearchVisualizationRequest(BaseModel):
    ...
    max_results: int = Field(default=50, ge=1, le=200, description="Maximum results for analysis")
...
async def get_search_visualization_stats(request: SearchVisualizationRequest):
    ...
    try:
        ...
        total_samples = sum(int(d.get('sample_count', 0) or 0) for d in metadata)
        unique_organisms = len(set(d.get('organism', 'Unknown') for d in metadata if d.get('organism')))
        unique_platforms = len(set(d.get('platform', 'Unknown') for d in metadata if d.get('platform')))
        avg_samples = int(total_samples / total_datasets) if total_datasets > 0 else 0
        
        # Calculate date range
        dates = [d.get('publication_date', '') for d in metadata if d.get('publication_date')]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get visualization stats: {str(e)}")


@visualization_router.post("/entity-distribution", response_model=EntityDistribution)
async def get_entity_distribution(request: SearchVisualizationRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get entity distribution: {str(e)}")


@visualization_router.post("/organism-distribution", response_model=OrganismDistribution)
async def get_organism_distribution(request: SearchVisualizationRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get organism distribution: {str(e)}")


@visualization_router.post("/platform-distribution", response_model=PlatformDistribution)
async def get_platform_distribution(request: SearchVisualizationRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get platform distribution: {str(e)}")


@visualization_router.post("/timeline-distribution", response_model=TimelineDistribution)
async def get_timeline_distribution(request: SearchVisualizationRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get timeline distribution: {str(e)}")
