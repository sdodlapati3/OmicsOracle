...
class PipelineMonitor:
    ...
    def get_query_events(self, query_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        query_events = [
            ...
            if event.detail.get("query_id") == query_id
        ]
...
async def monitor_pipeline_init(pipeline) -> None:
    ...
    # NCBI email
    if hasattr(pipeline, "config") and hasattr(pipeline.config, "ncbi") and hasattr(pipeline.config.ncbi, "email"):
...
async def monitor_query_complete(query_id: str, result: Any) -> None:
    ...
    await monitor.process_event(
        ...
        PipelineEvent(
            ...
            message=f"Query complete",
            percentage=100.0,
            detail=result_details
        )
    )
