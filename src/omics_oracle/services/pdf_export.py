...
class PDFExportService:
    ...
    def __init__(self):
        ...
        if not REPORTLAB_AVAILABLE:
            logger.warning(
                "ReportLab not available. PDF export will use fallback mode."
            )
    ...
    def generate_report(
        ...
        if output_path is None:
            ...
            output_path = Path(
                f"omics_oracle_report_{query_clean}_{timestamp}.pdf"
            )
    ...
    def _add_header(self, story: List, query_result: Dict[str, Any]):
        ...
        # Title
        title = Paragraph(
            "🧬 OmicsOracle Research Report", self.styles["CustomTitle"]
        )
        story.append(title)
    ...
    def _add_ai_insights(self, story: List, ai_summaries: Dict[str, Any]):
        ...
        # AI Insights header
        ai_header = Paragraph(
            "🤖 AI-Generated Insights", self.styles["Heading1"]
        )
    ...
    def _add_dataset_details(self, story: List, metadata: List[Dict[str, Any]]):
        ...
        datasets_header = Paragraph(
            "📚 Dataset Details", self.styles["Heading1"]
        )
    ...
    def _generate_fallback_report(
        ...
        if output_path is None:
            ...
            output_path = Path(
                f"omics_oracle_report_{query_clean}_{timestamp}.txt"
            )
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            # Query information
            f.write(
                f"Search Query: {query_result.get('original_query', 'N/A')}\n"
            )
            f.write(
                f"Results Found: {len(query_result.get('metadata', []))} datasets\n"
            )
            f.write(
                f"Processing Time: {query_result.get('processing_time', 0):.2f} seconds\n"
            )
            f.write(
            ...
            if query_result.get("metadata"):
                ...
                for i, dataset in enumerate(query_result["metadata"], 1):
                    ...
                    f.write(
                        f"   Organism: {dataset.get('organism', 'Not specified')}\n"
                    )
                    f.write(
                        f"   Summary: {dataset.get('summary', 'No summary')[:200]}...\n\n"
                    )
