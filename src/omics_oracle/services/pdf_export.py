...
class PDFExportService:
    ...
    def generate_report(
        ...
        if output_path is None:
            ...
            query_clean = "".join(c for c in query_result.get('original_query', 'query')[:20] if c.isalnum() or c in (' ', '-', '_')).replace(' ', '_')
    ...
    def _add_dataset_details(self, story: List, metadata: List[Dict[str, Any]]):
        ...
        for i, dataset in enumerate(metadata[:10], 1):  # Limit to first 10 datasets
            ...
            data = [
                ...
                ['Submission Date', dataset.get('submission_date', 'Not available')],
            ]
