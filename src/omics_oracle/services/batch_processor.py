...
class BatchProcessor:
    ...
    def create_batch_job(
        ...
        # Create batch job
        batch_job = BatchJob(id=job_id, name=job_name, queries=batch_queries)

        self.active_jobs[job_id] = batch_job

        logger.info(
            f"Created batch job '{job_name}' with {len(batch_queries)} queries"
        )
        return job_id
    ...
    async def process_batch_job(self, job_id: str) -> BatchJob:
        ...
        logger.info(
            f"Starting batch job {job_id} with {len(batch_job.queries)} queries"
        )
        ...
        try:
            ...
            logger.info(
                f"Batch job {job_id} completed: {batch_job.completed_queries} successful, {batch_job.failed_queries} failed"
            )
        ...
        async def process_single_query(batch_query: BatchQuery):
            async with semaphore:
                try:
                    ...
                    logger.info(
                        f"Query '{batch_query.query[:50]}...' completed in {processing_time:.2f}s"
                    )
                    ...
                    logger.error(
                        f"Query '{batch_query.query[:50]}...' failed: {e}"
                    )
    ...
    def export_batch_results(
        ...
        if format == "json":
            ...
            successful_queries = [
                q
                for q in batch_job.queries
                if q.status == BatchStatus.COMPLETED
            ]
            total_datasets = sum(
                len(q.result.get("geo_ids", []))
                for q in successful_queries
                if q.result
            )
            total_processing_time = sum(
                q.processing_time or 0 for q in batch_job.queries
            )
            ...
            return {
                "job_summary": {
                    ...
                    "average_time_per_query": total_processing_time
                    / batch_job.total_queries
                    if batch_job.total_queries > 0
                    else 0,
                },
                "query_results": [
                    {
                        "query": q.query,
                        "status": q.status.value,
                        "datasets_found": len(q.result.get("geo_ids", []))
                        if q.result
                        else 0,
                        "has_ai_summary": bool(
                            q.result and q.result.get("ai_summaries")
                        )
                        if q.result
                        else False,
                        "processing_time": q.processing_time,
