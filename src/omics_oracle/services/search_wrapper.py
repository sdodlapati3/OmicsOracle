...
class EnhancedSearchService:
    ...
    def __init__(self, *args, **kwargs):
        ...
        logger.info(
            f"NCBI_EMAIL environment variable: {ncbi_email or 'Not set'}"
        )
    ...
    def set_progress_callback(self, callback):
        ...
        def enhanced_callback(strategy_index, strategy_name, status):
            try:
                ...
                logger.info(
                    f"Progress: Strategy {strategy_index} - {strategy_name} - {status}"
                )
            except Exception as e:
    ...
    async def search_with_multiple_strategies(self, query, max_results=10):
        ...
        if not ncbi_email:
            ...
            try:
                ...
                if ncbi_email:
                    logger.info(
                        f"Loaded NCBI_EMAIL from .env.local: {ncbi_email}"
                    )
                else:
        ...
        if hasattr(self.service, "geo_client") and hasattr(
            self.service.geo_client, "_client"
        ):
            geo_client = self.service.geo_client

            # Try to access email attribute safely
            client_email = getattr(
                getattr(geo_client, "_client", None), "email", None
            )
            ...
            if (
                not client_email
                and ncbi_email
                and hasattr(geo_client, "_client")
            ):
                try:
        ...
        try:
            ...
            result = await self.service.search_with_multiple_strategies(
            ...
            logger.info(
                f"Search completed in {elapsed:.2f}s with {len(geo_ids)} results"
            )
