...
class SummaryCache:
    ...
    def __init__(self, cache_dir: Optional[Path] = None, ttl_hours: int = 168):  # 1 week default
        """
    ...
    def _generate_cache_key(self, query: str, summary_type: str, 
    ...
    def get(self, query: str, summary_type: str, 
    ...
    def set(self, query: str, summary_type: str, summary_data: Dict[str, Any],
            max_results: int = 10, token_count: int = 0):
        """
        ...
        logger.info(f"Cached AI summary for query: {query[:50]}... (tokens: {token_count})")
    
    def _delete_expired_entry(self, cache_key: str):
        """Delete an expired cache entry."""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("DELETE FROM ai_summaries WHERE query_hash = ?", (cache_key,))
            conn.commit()
