...
class CostManager:
        ...
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    record.tokens_used, record.cost_usd, record.query,
                    record.response_size, record.user_id, record.session_id
                ))
                
        logger.info(f"Usage recorded: {service} - {tokens_used} tokens - ${cost_usd:.4f}")
        return cost_usd
    ...
    def _calculate_cost(self, tokens_used: int, model: str) -> float:
        ...
        if model in self.cost_rates:
            rate = (self.cost_rates[model]["input"] + self.cost_rates[model]["output"]) / 2
        else:
            # Default to GPT-4 rates
            rate = (self.cost_rates["gpt-4"]["input"] + self.cost_rates["gpt-4"]["output"]) / 2
    ...
    def check_limits(self, user_id: str = "anonymous") -> Dict[str, Any]:
        ...
        if stats.daily_tokens > self.daily_limits["tokens"]:
            daily_violations.append(f"Daily token limit exceeded: {stats.daily_tokens}/{self.daily_limits['tokens']}")
            
        if stats.daily_cost_usd > self.daily_limits["cost_usd"]:
            daily_violations.append(f"Daily cost limit exceeded: ${stats.daily_cost_usd:.2f}/${self.daily_limits['cost_usd']}")
            
        if stats.daily_requests > self.daily_limits["requests"]:
            daily_violations.append(f"Daily request limit exceeded: {stats.daily_requests}/{self.daily_limits['requests']}")
            
        # Check monthly limits
        monthly_violations = []
        if stats.current_month_cost_usd > self.monthly_limits["cost_usd"]:
            monthly_violations.append(f"Monthly cost limit exceeded: ${stats.current_month_cost_usd:.2f}/${self.monthly_limits['cost_usd']}")
            
        return {
            "within_limits": len(daily_violations) == 0 and len(monthly_violations) == 0,
    ...
    def export_usage_data(self, output_path: Path, user_id: str = None, days: int = 30):
        """Export usage data to JSON file."""
        history = self.get_usage_history(user_id=user_id, days=days, limit=10000)
