...
class SummarizationService:
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        if not api_key:
            logger.warning("OpenAI API key not found. Summarization will use fallback mode.")
            self.client = None
        else:
    ...
    def summarize_dataset(
        ...
        cache_key = f"{dataset_id}_{query_context or 'no_context'}_{summary_type}"
        ...
        try:
            ...
            if summary_type == "comprehensive":
                summaries["technical_details"] = self._generate_technical_summary(
                    cleaned_metadata
                )
            
            # Cache the summary (estimate tokens used)
            estimated_tokens = sum(len(str(v).split()) * 1.3 for v in summaries.values())
            self.cache.set(cache_key, "dataset_summary", summaries, token_count=int(estimated_tokens))
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": ("You are a genomics research expert who creates clear, "
            ...
            return content.strip() if content else f"Dataset {metadata.get('accession', 'Unknown')}: {metadata.get('title', 'No title available')}"
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": ("You are a genomics methods expert. Provide clear, "
            ...
            content = response.choices[0].message.content
            return content.strip() if content else f"Experimental methodology using {metadata.get('platform', 'unknown platform')} technology."
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": ("You are a genomics research analyst who identifies "
            ...
            content = response.choices[0].message.content
            return content.strip() if content else "This dataset contributes to our understanding of genomic mechanisms and biological processes."
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": ("You are a genomics expert who creates concise, "
            ...
            content = response.choices[0].message.content
            return content.strip() if content else f"{metadata.get('accession', 'Dataset')}: {metadata.get('title', 'Genomics study')} using {metadata.get('platform', 'genomics technology')}."
    ...
    def _generate_technical_summary(self, metadata: Dict[str, Any]) -> str:
        ...
        return " | ".join(technical_info) if technical_info else "Technical details not available"

    def _build_overview_prompt(
        self, metadata: Dict[str, Any], query_context: Optional[str], brief: bool = False
    ) -> str:
    ...
    def _generate_fallback_summary(self, metadata: Dict[str, Any]) -> Dict[str, str]:
    ...
    def summarize_batch_results(
        ...
        cached_summary = self.cache.get(cache_key, "batch_summary", len(results))
        if cached_summary:
            logger.info(f"Using cached batch summary for query: {query[:50]}...")
            return cached_summary
        ...
        summary = {
            ...
            "overview": (f"Found {total_datasets} datasets with {total_samples} total samples "
