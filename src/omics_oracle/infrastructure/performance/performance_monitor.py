...
class PerformanceMonitor:
    ...
    def record_memory_metrics(self) -> None:
        ...
        for i, stats in enumerate(gc_stats):
            self.record_metric(
                f"gc_generation_{i}_collections", stats["collections"]
            )
            self.record_metric(
                f"gc_generation_{i}_collected", stats["collected"]
            )
            self.record_metric(
                f"gc_generation_{i}_uncollectable", stats["uncollectable"]
            )
                ...
                def sync_wrapper(*args, **kwargs):
                    ...
                    try:
                        ...
                        self.monitor.record_metric(
                            f"{name}_duration", duration, tags
                        )
                        return result
                        ...
                        self.monitor.record_metric(
                            f"{name}_error", duration, error_tags
                        )
                        raise
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                async def async_wrapper(*args, **kwargs):
                    ...
                    try:
                        ...
                        duration = time.time() - start_time
                        self.monitor.record_database_metrics(
                        ...
                        duration = time.time() - start_time
                        self.monitor.record_database_metrics(
                            f"{query_type}_error", table, duration
                        )
                        raise
