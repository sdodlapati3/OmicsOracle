...
class DatasetNotFoundError(RepositoryError):
    ...
    def __init__(self, geo_id: str, details: Optional[Dict[str, Any]] = None):
        ...
        super().__init__(message, operation="get", resource=geo_id, details=details)
        self.geo_id = geo_id
...
class ConcurrencyError(DomainError):
    ...
    def __init__(self, resource: str, message: str = "Resource is being modified by another process",
                 details: Optional[Dict[str, Any]] = None):
        super().__init__(f"Concurrency error for '{resource}': {message}", details)
        self.resource = resource
...
class DataIntegrityError(DomainError):
    ...
    def __init__(self, constraint: str, message: str,
                 details: Optional[Dict[str, Any]] = None):
        super().__init__(f"Data integrity error '{constraint}': {message}", details)
...
class NetworkError(ExternalServiceError):
    ...
    def __init__(self, operation: str, url: str, message: str,
                 details: Optional[Dict[str, Any]] = None):
        super().__init__("Network", f"{operation} failed for {url}: {message}", details=details)
        self.operation = operation
        self.url = url
...
class ParseError(DomainError):
    ...
    def __init__(self, data_type: str, message: str, data_sample: Optional[str] = None,
                 details: Optional[Dict[str, Any]] = None):
...
class QuotaExceededError(DomainError):
    ...
    def __init__(self, resource: str, quota: int, current_usage: int,
                 details: Optional[Dict[str, Any]] = None):
...
EXTERNAL_SERVICE_EXCEPTIONS = (ExternalServiceError, NetworkError, ServiceUnavailableError)
