...
async def download_export(export_id: str):
    ...
    if "file_path" not in job:
        raise HTTPException(
            status_code=500, detail="Export file path not found"
        )
    ...
    try:
        ...
        if export_format == "pdf":
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"Results: {len(query_result.get('metadata', []))} datasets\n\n"
                )

                if (
                    query_result.get("ai_summaries")
                    and job["include_ai_summaries"]
                ):
                    f.write("AI Insights:\n")
                    f.write("-" * 20 + "\n")
                    import json
                ...
                if query_result.get("metadata") and job["include_metadata"]:
                    ...
                    for i, dataset in enumerate(query_result["metadata"], 1):
                        ...
                        f.write(
                            f"   Summary: {dataset.get('summary', 'No summary')}\n\n"
                        )
            file_path = output_path
