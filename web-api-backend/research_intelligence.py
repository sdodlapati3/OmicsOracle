...
class ResearchIntelligenceEngine:
    ...
    def __init__(self):
        ...
        self.methodology_success_rates: Dict[
            str, Dict[str, float]
        ] = defaultdict(dict)
    ...
    async def identify_research_gaps(
        ...
        logger.info(
            f"Identifying research gaps for context: {research_context}"
        )
        ...
        technique_gaps = await self._identify_technique_disease_gaps(
            research_context
        )
    ...
    def _find_underexplored_entities(self) -> List[Tuple[str, float]]:
        ...
        for name, entity in self.entity_knowledge_base.items():
            ...
            foundational_score = (
                math.log(entity.publications_count + 1) * 0.3
                + math.log(entity.datasets_count + 1) * 0.7
            )
    ...
    async def _identify_technique_disease_gaps(
        ...
        diseases = [
            e
            for e in self.entity_knowledge_base.values()
            if e.category == "disease"
        ]
        techniques = [
            e
            for e in self.entity_knowledge_base.values()
            if e.category == "technique"
        ]
    ...
    async def discover_cross_domain_connections(
        ...
        domain_pairs = [
            (d1, d2) for i, d1 in enumerate(domains) for d2 in domains[i + 1 :]
        ]

        for domain_a, domain_b in domain_pairs:
            connection = await self._analyze_domain_connection(
                domain_a, domain_b, domain_entities
            )
    ...
    async def _analyze_domain_connection(
        ...
        for entity_a in entities_a[:5]:  # Limit for performance
            for entity_b in entities_b[:5]:
                ...
                connection_score = self._calculate_entity_connection(
                    entity_a, entity_b
                )
        ...
        novelty_score = 1.0 - (
            avg_strength * 0.5
        )  # Higher novelty for weaker existing connections
    ...
    def _calculate_entity_connection(
        self, entity_a: ResearchEntity, entity_b: ResearchEntity
    ) -> float:
        ...
        activity_correlation = min(
            entity_a.recent_activity, entity_b.recent_activity
        )
        frequency_correlation = min(
            entity_a.frequency, entity_b.frequency
        ) / max(entity_a.frequency, entity_b.frequency)
    ...
    def _calculate_technique_relevance(
        ...
        if any(
            ...
            for keyword in goal_keywords.split()
        ):
            keyword_bonus = 0.3
    ...
    def _calculate_gap_confidence(
        self, entity: ResearchEntity, gap_score: float
    ) -> float:
        """Calculate confidence level for a research gap."""
        data_quality = (
            min(entity.publications_count / 1000, 1.0) * 0.4
            + min(entity.datasets_count / 100, 1.0) * 0.6
        )
...
class PersonalizationEngine:
    ...
    def __init__(self):
        ...
        self.interaction_history: Dict[str, List[Dict[str, Any]]] = defaultdict(
            list
        )
    ...
    def get_personalized_recommendations(
        ...
        for insight in base_insights:
            relevance_score = self._calculate_insight_relevance(
                insight, profile
            )
    ...
    def _calculate_insight_relevance(
        self, insight: ResearchInsight, profile: Dict[str, Any]
    ) -> float:
        ...
        entity_matches = len(
            set(insight.entities) & profile["research_interests"]
        )
