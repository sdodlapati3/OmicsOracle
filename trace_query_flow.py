...
async def trace_query_flow(
    query: str, max_results: int = 10, output_dir: str = "query_traces"
):
    """
    ...
    try:
        ...
        with tracer.trace_context("query_parsing", "query_analyzer"):
            ...
            try:
                ...
                try:
                    alt_queries = parser.generate_alternative_queries(
                        components
                    )
                    tracer.record_step(
                        "query_parsing",
        ...
        with tracer.trace_context("search", "pipeline_processing"):
            ...
            try:
                ...
                if geo_ids:
                    result = type(
                        ...
                        (),
                        {
                            ...
                            "ai_summaries": metadata_info.get(
                                "ai_summaries", {}
                            ),
                    ...
                    logger.info(
                        "Enhanced search found no results, using standard pipeline"
                    )
                ...
                logger.info(
                    "Enhanced search module not available, using standard pipeline"
                )
                result = await pipeline.process_query(
                    query, max_results=max_results
                )
            ...
            if hasattr(result, "geo_ids") and result.geo_ids:
                for i, geo_id in enumerate(result.geo_ids[:max_results]):
                    ...
                    if (
                        ...
                        for summary_item in result.ai_summaries[
            ...
            tracer.record_step(
                ...
                "response_generation",
                {
                    "result_obj": {
                        ...
                        "geo_id_count": len(result.geo_ids)
                        ...
                        "has_ai_summaries": hasattr(result, "ai_summaries")
                        and bool(result.ai_summaries),
                    }
                },
        ...
        report_path = os.path.join(output_dir, f"{trace_id}_report.md")
        logger.error(
            f"Query trace failed. Error report saved to: {report_path}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Trace a query's flow through OmicsOracle"
    )
