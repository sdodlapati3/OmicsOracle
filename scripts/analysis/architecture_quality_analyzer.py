...
class ArchitecturalAnalyzer:
    ...
    def _analyze_file(self, file_path: Path):
        ...
        try:
            ...
            god_class_score = self._calculate_god_class_score(analyzer.class_methods)
    ...
    def _calculate_god_class_score(self, class_methods: Dict[str, List[str]]) -> float:
    ...
    def _analyze_components(self):
        ...
        for component_name, files in components.items():
            ...
            # Calculate totals
            metrics.total_loc = sum(self.file_metrics[f].lines_of_code for f in files)

            # Calculate average complexity
            complexities = [self.file_metrics[f].cyclomatic_complexity for f in files]
            metrics.average_complexity = sum(complexities) / len(complexities) if complexities else 0
    ...
    def _calculate_cohesion(self, files: List[str]) -> float:
        ...
        cohesion = (internal_deps / total_possible) * 10.0 if total_possible > 0 else 10.0
    ...
    def _identify_violations(self, files: List[str]) -> List[str]:
        ...
        for file_path in files:
            ...
            if metrics.lines_of_code > 500:
                violations.append(f"God file: {file_path} ({metrics.lines_of_code} LOC)")

            if metrics.cyclomatic_complexity > 50:
                violations.append(f"High complexity: {file_path} ({metrics.cyclomatic_complexity})")
    ...
    def _generate_analysis(self) -> ArchitecturalAnalysis:
        ...
        analysis.total_loc = sum(m.lines_of_code for m in self.file_metrics.values())

        # Find circular dependencies
        try:
            cycles = list(nx.simple_cycles(self.dependency_graph))
            analysis.circular_dependencies = [(cycle[0], cycle[1]) for cycle in cycles[:10]]  # Limit to 10
        except Exception as e:
            logger.warning(f"Failed to analyze circular dependencies: {e}")
            analysis.circular_dependencies = []

        # Count violations
        analysis.sys_path_violations = sum(m.sys_path_usage for m in self.file_metrics.values())
        analysis.god_files = [m.path for m in self.file_metrics.values() if m.lines_of_code > 500]
    ...
    def _calculate_architectural_score(self) -> float:
        ...
        avg_file_size = sum(m.lines_of_code for m in self.file_metrics.values()) / len(self.file_metrics)
        size_score = max(0, 10 - (avg_file_size / 100))  # Penalize files >1000 LOC heavily
        scores.append(size_score)

        # Complexity score
        avg_complexity = sum(m.cyclomatic_complexity for m in self.file_metrics.values()) / len(
            self.file_metrics
        )
        ...
        sys_path_violations = sum(m.sys_path_usage for m in self.file_metrics.values())
        ...
        try:
            ...
            logger.warning(f"Failed to count circular dependencies: {e}")
            cycle_count = 0
    ...
    def _generate_recommendations(self, analysis: ArchitecturalAnalysis) -> List[str]:
