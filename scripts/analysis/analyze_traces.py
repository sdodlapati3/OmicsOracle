...
def load_all_traces(trace_dir: str, query: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    ...
    for file_path in trace_files:
        ...
        if query and query.lower() not in trace_data.get("original_query", "").lower():
            continue
    ...
    logger.info(f"Loaded {len(traces)} trace files" + (f" for query containing '{query}'" if query else ""))
...
def generate_flow_document(flow_data: Dict[str, Any], output_file: str) -> None:
    ...
    with open(output_file, "w") as f:
        ...
        f.write(f"This document analyzes {len(flows)} query flows through the OmicsOracle system.\n\n")
        ...
        successful_flows = sum(1 for flow in flows if flow.get("status") == "completed")
        failed_flows = sum(1 for flow in flows if flow.get("status") == "failed")

        avg_duration = sum(flow.get("duration", 0) for flow in flows) / len(flows) if flows else 0
        avg_results = sum(len(flow.get("geo_ids", [])) for flow in flows) / len(flows) if flows else 0
        ...
        if all_components:
            ...
            for component in sorted(all_components):
                ...
                flows_with_component = [
                    flow for flow in flows if component in flow.get("component_times", {})
                ]

                avg_time = sum(
                    flow["component_times"][component]["total_time"] for flow in flows_with_component
                ) / len(flows_with_component)

                avg_calls = sum(
                    flow["component_times"][component]["calls"] for flow in flows_with_component
                ) / len(flows_with_component)

                f.write(f"| {component} | {avg_time:.3f} | {avg_calls:.1f} | {len(flows_with_component)} |\n")
        ...
        for i, flow in enumerate(flows, 1):
            ...
            f.write(f"### <a name='flow-{i}'></a>{status_emoji} Flow {i}: {query}\n\n")
            ...
            if steps:
                ...
                key_steps = [
                    step for step in steps if step.get("stage") in key_stages or step.get("status") == "error"
                ]
                ...
                for step in key_steps:
                    ...
                    f.write(f"**{status_icon} Step {step_id}: {stage} - {component}**\n\n")
        ...
        for stage in sorted(all_stages):
            steps_in_stage = sum(
                1 for flow in flows for step in flow.get("steps", []) if step.get("stage") == stage
            )

            flows_with_stage = sum(
                1 for flow in flows if any(step.get("stage") == stage for step in flow.get("steps", []))
            )
        ...
        for component in sorted(all_components):
            steps_with_component = sum(
                1 for flow in flows for step in flow.get("steps", []) if step.get("component") == component
            )

            flows_with_component = sum(
                1
                for flow in flows
                if any(step.get("component") == component for step in flow.get("steps", []))
            )
        ...
        if stage_durations:
            ...
            f.write("| Stage | Avg. Duration (s) | Min (s) | Max (s) | Count |\n")
            f.write("|-------|-------------------|---------|---------|-------|\n")
...
def analyze_traces(trace_dir: str, output_file: str, query: Optional[str] = None) -> None:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze OmicsOracle query traces")
    parser.add_argument(
        "--trace-dir",
        default="query_traces",
        help="Directory containing trace files",
    )
    parser.add_argument("--output", default="query_flow_analysis.md", help="Output file path")
