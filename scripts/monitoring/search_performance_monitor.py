...
class SearchPerformanceMonitor:
    ...
    async def measure_query_performance(self, query: str, iterations: int = 3) -> Dict[str, Any]:
        """
                ...
                try:
                    ...
                    async with session.get(self.search_endpoint, params=params) as response:
                        ...
                        if response.status == 200:
                            ...
                            metrics["result_counts"].append(len(data.get("results", [])))

                            # Extract resource usage if available
                            metrics["memory_usage"].append(perf_data.get("memory_usage", 0))
                            metrics["cpu_usage"].append(perf_data.get("cpu_usage", 0))
                        else:
                            logger.warning(
                    ...
                    logger.error(f"Error measuring performance for query '{query}': {str(e)}")
    ...
    async def run_performance_test(self, queries: List[str], iterations: int = 3) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Running performance test on {len(queries)} queries with {iterations} iterations each")
        ...
        response_times = [
            m["avg_response_time"] for m in all_metrics["queries"].values() if "avg_response_time" in m
        ]

        if response_times:
            all_metrics["summary"]["overall_avg_response_time"] = sum(response_times) / len(response_times)
            all_metrics["summary"]["overall_min_response_time"] = min(response_times)
            all_metrics["summary"]["overall_max_response_time"] = max(response_times)
            all_metrics["summary"]["response_time_std_dev"] = np.std(response_times)
    ...
    def generate_visualizations(
        ...
        avg_times = [metrics["queries"][q].get("avg_response_time", 0) for q in queries]
        ...
        for query, data in metrics["queries"].items():
            if data.get("component_times") and len(data["component_times"]) > 0:
                ...
                avg_times = [sum(avg_component_times[c]) / len(avg_component_times[c]) for c in components]
                ...
                component_path = output_dir / f"components_{query_name}_{timestamp}.png"
    ...
    async def run_and_save_report(
        ...
        if save_json:
            json_path = self.results_dir / f"search_performance_{timestamp}.json"
        ...
        if save_markdown:
            ...
            md_path = self.results_dir / f"search_performance_report_{timestamp}.md"
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="OmicsOracle Search Performance Monitor")
    parser.add_argument(
        "--url",
    ...
    parser.add_argument("--queries-file", help="JSON file with custom test queries")
    parser.add_argument("--no-json", action="store_true", help="Don't save raw metrics as JSON")
    parser.add_argument(
        "--no-markdown",
        action="store_true",
        ...
        try:
            with open(args.queries_file, "r") as f:
                ...
                if not isinstance(test_queries, list):
                    logger.error("Queries file must contain a JSON array of query strings")
                    return
        except Exception as e:
