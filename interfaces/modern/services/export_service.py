...
class ExportService:
    ...
    def export_search_results(
        ...
        """
        try:
            self.logger.info(f"Starting export: {len(results)} results to {export_request.format}")
            ...
            filename = f"omics_oracle_export_{timestamp}.{export_request.format}"
            ...
            if export_request.format == 'csv':
                ...
                raise ExportException(f"Unsupported export format: {export_request.format}")
            ...
            self.logger.info(f"Export completed: {filename} ({len(results)} records)")
    ...
    def _prepare_row_data(self, result: SearchResult, fields: List[str]) -> Dict[str, Any]:
    ...
    def cleanup_expired_exports(self) -> int:
        ...
        try:
            ...
            if cleaned_count > 0:
                self.logger.info(f"Cleaned up {cleaned_count} expired export files")
