...
class SearchService:
    ...
    async def search(self, query: SearchQuery) -> SearchResponse:
        ...
        try:
            self.logger.info(f"Processing search: '{query.query}' (page: {query.page}, size: {query.page_size})")
            ...
            if hasattr(pipeline, 'search_datasets'):
                ...
                pipeline_results = await pipeline.search_datasets(
                    query.query,
                    max_results=min(query.page_size * 2, 100)
                )
            else:
            ...
            if not pipeline_results or not hasattr(pipeline_results, 'metadata'):
            ...
            total_pages = (total_available + query.page_size - 1) // query.page_size
            execution_time = time.time() - start_time
            
            self.logger.info(f"Search completed: {len(search_results)} results in {execution_time:.3f}s")
            ...
            self.logger.error(f"Search failed for query '{query.query}': {str(e)}", exc_info=True)
    ...
    def _validate_query(self, query: SearchQuery) -> None:
        ...
        if len(query.query) > 1000:
            raise ValidationException("Search query is too long (max 1000 characters)")
    ...
    def _process_result(self, result_data: Any, ai_summaries: Dict[str, Any]) -> SearchResult:
        """Process a single search result"""
        try:
            # Extract basic information
            geo_id = self._extract_field(result_data, ['geo_id', 'id', 'accession']) or "unknown"
            title = self._extract_field(result_data, ['title', 'name']) or "Untitled"
            abstract = self._extract_field(result_data, ['summary', 'abstract', 'description'])
            ...
            authors = self._extract_field(result_data, ['authors', 'contributor']) or []
    ...
    def _extract_field(self, data: Any, field_names: List[str]) -> Optional[str]:
        ...
        if text_content:
            ...
            for pattern, organism_name in self.organism_patterns.items():
                if re.search(pattern, text_lower):
                    self.logger.debug(f"Extracted organism from text: {organism_name}")
                    return organism_name
            
            # Check for human indicators
            if any(indicator in text_lower for indicator in self.human_indicators):
    ...
    def _extract_sample_count(self, data: Any) -> str:
        ...
        sample_count = self._extract_field(data, ['sample_count', 'n_samples', 'samples'])
    ...
    def _get_text_content(self, data: Any) -> str:
        ...
        for field in ['summary', 'title', 'overall_design', 'description', 'abstract']:
    ...
    def _extract_ai_summary(self, result_data: Any, ai_summaries: Dict[str, Any]) -> str:
