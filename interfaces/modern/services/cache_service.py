...
class CacheService:
    ...
    def __init__(self, cache_dir: Path, ttl: int = 3600, enabled: bool = True):
        ...
        if self.enabled:
            ...
            self.logger.info(f"Cache service initialized: {self.cache_dir} (TTL: {self.ttl}s)")
        ...
        try:
            ...
            for cache_file in self.cache_dir.glob('*.json'):
                try:
                    ...
                    self.logger.warning(f"Failed to check cache file {cache_file}: {str(e)}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if cache_files:
                ...
                stats['oldest_entry'] = datetime.fromtimestamp(oldest_file.stat().st_mtime).isoformat()
                stats['newest_entry'] = datetime.fromtimestamp(newest_file.stat().st_mtime).isoformat()
    ...
    def cache_search_result(self, query: str, page: int, page_size: int, result: Any) -> bool:
    ...
    def get_cached_search_result(self, query: str, page: int, page_size: int) -> Optional[Any]:
