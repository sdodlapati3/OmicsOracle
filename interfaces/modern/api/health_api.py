...
def detailed_health_check():
    ...
    try:
        ...
        try:
            cache_accessible = config.CACHE_DIR.exists() and config.CACHE_DIR.is_dir()
            components['cache'] = {
        ...
        try:
            data_accessible = config.DATA_DIR.exists() and config.DATA_DIR.is_dir()
            components['data'] = {
...
def readiness_check():
    ...
    try:
        ...
        try:
            ...
            checks.append({'component': 'configuration', 'status': 'not_ready', 'error': str(e)})
            ready = False
        ...
        for dir_path in required_dirs:
            try:
                ...
                if dir_path.exists():
                    checks.append({'component': f'directory_{dir_path.name}', 'status': 'ready'})
                else:
                    checks.append({'component': f'directory_{dir_path.name}', 'status': 'not_ready'})
                    ready = False
            except Exception as e:
                checks.append({'component': f'directory_{dir_path.name}', 'status': 'not_ready', 'error': str(e)})
                ready = False
