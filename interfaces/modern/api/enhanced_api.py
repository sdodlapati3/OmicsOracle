...
def get_quick_filters():
    ...
    try:
        return jsonify({"filters": QUICK_FILTERS, "count": len(QUICK_FILTERS)})
...
def get_search_suggestions():
    ...
    try:
        ...
        for history_item in search_history[-10:]:  # Last 10 searches
            if (
                query in history_item.lower()
                and history_item not in suggestions
            ):
...
def get_example_searches():
    ...
    try:
        return jsonify(
            {"examples": EXAMPLE_SEARCHES, "count": len(EXAMPLE_SEARCHES)}
        )
    except Exception as e:
...
def get_search_history():
    ...
    try:
        ...
        return jsonify(
            {"history": recent_history, "count": len(recent_history)}
        )
    except Exception as e:
...
def add_search_to_history():
    ...
    try:
        ...
        return jsonify({"message": "Query added to history", "query": query})
