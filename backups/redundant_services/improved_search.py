...
class ImprovedSearchService:
    ...
    def enhance_entity_extraction(self, query: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for technique, synonyms in self.experimental_techniques.items():
            if technique in query_lower or any(syn.lower() in query_lower for syn in synonyms):
                ...
                existing_texts = {e["text"].lower() for e in enhanced_entities["experimental_techniques"]}
        ...
        if "general" in enhanced_entities:
            ...
            for item in general_items:
                ...
                for technique, synonyms in self.experimental_techniques.items():
                    if text_lower == technique or text_lower in [s.lower() for s in synonyms]:
                        if "experimental_techniques" not in enhanced_entities:
                            enhanced_entities["experimental_techniques"] = []
                        enhanced_entities["experimental_techniques"].append({**item, "label": "TECHNIQUE"})
                ...
                if not moved:
                    ...
                    if any(term in text_lower for term in anatomy_terms):
                        ...
                        enhanced_entities["tissues"].append({**item, "label": "TISSUE"})
                        enhanced_entities["general"].remove(item)

        return enhanced_entities

    def create_search_strategies(self, entities: Dict[str, List[Dict[str, Any]]]) -> List[Tuple[str, str]]:
        """Create multiple search strategies for better coverage."""
        strategies = []

        # Extract entity texts by category
        techniques = [e["text"] for e in entities.get("experimental_techniques", [])]
        ...
        if techniques and diseases:
            ...
            if tech_term in self.experimental_techniques:
                core_terms.extend(self.experimental_techniques[tech_term][:2])  # Top 2 synonyms
            else:
            ...
            if disease_term in self.disease_tissue_associations:
                core_terms.extend(self.disease_tissue_associations[disease_term][:2])
    ...
    async def search_with_multiple_strategies(
        ...
        for strategy_name, search_query in strategies:
            try:
                logger.info(f"Trying strategy '{strategy_name}': {search_query}")
                results = await self.geo_client.search_geo_series(search_query, max_results=max_results)
                ...
                if results:
                    ...
                    logger.info(f"Strategy '{strategy_name}' found {len(results)} results")
                else:
    ...
    for query in test_queries:
        ...
        try:
            ...
            for strategy, details in metadata["strategy_details"].items():
                print(f"  - {strategy}: '{details['query']}' -> {details['count']} results")
