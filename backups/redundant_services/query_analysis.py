...
class QueryAnalysisService:
    ...
    def __init__(self, biomedical_ner=None, synonym_mapper=None):
        ...
        self.query_patterns = {
            "technical_assay": re.compile(r"\b(rna-seq|microarray|chip-seq|wgbs|atac-seq)\b", re.I),
            "organism": re.compile(r"\b(human|mouse|rat|homo sapiens|mus musculus)\b", re.I),
            "tissue_type": re.compile(r"\b(brain|liver|heart|lung|kidney|blood|skin)\b", re.I),
            "disease_terms": re.compile(r"\b(cancer|tumor|disease|syndrome|disorder)\b", re.I),
        }
    ...
    def analyze_failed_query(self, query: str, result_count: int) -> QueryAnalysis:
        ...
        """
        logger.debug(f"Analyzing failed query: '{query}' with {result_count} results")
        ...
        modifications = self._generate_query_modifications(query, entities, issues)
        ...
        logger.debug(f"Query analysis completed: {len(issues)} issues identified")
        return analysis
    ...
    def generate_suggestions(self, analysis: QueryAnalysis) -> List[QuerySuggestion]:
        ...
        for issue in analysis.potential_issues:
            if issue == QueryIssue.TOO_SPECIFIC:
                suggestions.extend(self._generate_broadening_suggestions(analysis))
                ...
                suggestions.extend(self._generate_term_addition_suggestions(analysis))
            elif issue == QueryIssue.MISSPELLED_TERMS:
                suggestions.extend(self._generate_spelling_suggestions(analysis))
    ...
    def find_similar_successful_queries(self, query: str, limit: int = 5) -> List[SimilarQuery]:
        """
        ...
        similar_queries.sort(key=lambda x: x.similarity_score * x.success_score, reverse=True)
    ...
    def _calculate_complexity_score(self, query: str, entities: Dict[str, List]) -> float:
        ...
        technical_patterns = sum(1 for pattern in self.query_patterns.values() if pattern.search(query))
        score += min(technical_patterns / 10.0, 0.3)  # Max 0.3 for technical terms
    ...
    def _identify_query_issues(
        ...
        if total_entities == 0 and not any(term in query.lower() for term in self.common_biomedical_terms):
    ...
    def _has_rare_entities(self, entities: Dict[str, List]) -> bool:
        ...
        for entity_list in entities.values():
            for entity in entity_list:
                ...
                if any(indicator in entity_text for indicator in rare_indicators):
                    return True
        return False
    ...
    def _has_potential_misspellings(self, query: str) -> bool:
        ...
        for word in words:
            if len(word) > 6 and not any(re.match(pattern, word) for pattern in suspicious_patterns):
                # Very basic spell check - in production use proper spell checker
                if word not in self.common_biomedical_terms:
                    return True
        return False

    def _calculate_entity_rarity_scores(self, entities: Dict[str, List]) -> Dict[str, float]:
        ...
        for entity_type, entity_list in entities.items():
            for entity in entity_list:
                ...
                if len(entity_text.split()) > 2:  # Multi-word entities are often rarer
                    score = 0.6
                if any(rare_term in entity_text.lower() for rare_term in ["syndrome", "mutation", "variant"]):
                    score = 0.3
    ...
    def _generate_query_modifications(
        ...
        if QueryIssue.NO_BIOMEDICAL_ENTITIES in issues:
            modifications.append("Add biological context (e.g., human, gene, disease)")

        return modifications

    def _generate_broadening_suggestions(self, analysis: QueryAnalysis) -> List[QuerySuggestion]:
    ...
    def _generate_synonym_suggestions(self, analysis: QueryAnalysis) -> List[QuerySuggestion]:
        ...
        for entity_type, entity_list in analysis.entities_found.items():
            for entity in entity_list:
                entity_text = entity.get("text", "")
                try:
                    synonyms = self.synonym_mapper.get_synonyms(entity_text, entity_type)
                    if synonyms:
                        # Take the first synonym as a suggestion
                        synonym = list(synonyms)[0]
                        new_query = analysis.original_query.replace(entity_text, synonym)
                    ...
                    logger.debug(f"Synonym lookup failed for '{entity_text}': {e}")

        return suggestions

    def _generate_term_addition_suggestions(self, analysis: QueryAnalysis) -> List[QuerySuggestion]:
    ...
    def _generate_spelling_suggestions(self, analysis: QueryAnalysis) -> List[QuerySuggestion]:
        """Generate spelling correction suggestions."""
        # This would integrate with a spell checker in production
        return []

    def _generate_simplification_suggestions(self, analysis: QueryAnalysis) -> List[QuerySuggestion]:
    ...
    def _generate_structural_suggestions(self, analysis: QueryAnalysis) -> List[QuerySuggestion]:
    ...
    def _extract_key_terms(self, query: str) -> List[str]:
        ...
        biomedical_terms = [term for term in key_terms if term in self.common_biomedical_terms]
        other_terms = [term for term in key_terms if term not in self.common_biomedical_terms]
    ...
    def _find_common_entities(self, query1: str, query2: str) -> List[str]:
        ...
        biomedical_common = [
            word for word in common_words if word in self.common_biomedical_terms or len(word) > 4
        ]
