...
def validate_search_input(request: SearchRequest):
    ...
    for pattern in sql_patterns:
        if re.search(pattern, query_lower):
            raise HTTPException(status_code=400, detail="Invalid characters detected in query")
...
async def get_dataset_info(dataset_id: str, include_sra: bool = False):
    ...
    try:
        ...
        result = await pipeline.get_dataset_info(dataset_id=dataset_id, include_sra=include_sra)
