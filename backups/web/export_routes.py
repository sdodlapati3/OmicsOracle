...
async def create_export(request: ExportRequest, background_tasks: BackgroundTasks):
    ...
    try:
        # Generate export ID
        export_id = (
            f"export_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{hash(str(request.query_result)) % 10000:04d}"
        )
...
async def download_export(export_id: str):
    ...
    if "file_path" not in job:
        raise HTTPException(status_code=500, detail="Export file path not found")
