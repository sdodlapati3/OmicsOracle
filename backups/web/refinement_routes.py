...
from ..services.query_analysis import QueryAnalysisService, QuerySuggestion, SimilarQuery, SuggestionType
from ..web.models import QueryStatus

logger = logging.getLogger(__name__)

# Create router for refinement endpoints
refinement_router = APIRouter(prefix="/api/refinement", tags=["Query Refinement"])
...
class QuerySuggestionResponse(BaseModel):
    ...
    suggestions: List[Dict] = Field(..., description="List of query suggestions")
    alternative_queries: List[str] = Field(default=[], description="Alternative successful queries")
    explanation: str = Field(..., description="Explanation of suggestion generation")
    analysis_metadata: Dict = Field(default={}, description="Additional analysis metadata")
...
class SimilarQueriesRequest(BaseModel):
    ...
    limit: int = Field(default=5, description="Maximum number of similar queries")


class SimilarQueriesResponse(BaseModel):
    """Response model for similar queries."""

    similar_queries: List[Dict] = Field(..., description="List of similar successful queries")
    success_patterns: List[str] = Field(default=[], description="Common success patterns")
...
class QueryFeedbackRequest(BaseModel):
    ...
    suggested_query: str = Field(..., description="Suggested query that was evaluated")
    user_action: str = Field(..., description="Action taken by user")
    was_helpful: bool = Field(..., description="Whether suggestion was helpful")
    result_improvement: Optional[int] = Field(None, description="Improvement in result count")
    user_session_id: Optional[str] = Field(None, description="User session ID")
...
class EnhancedSearchRequest(BaseModel):
    ...
    include_suggestions: bool = Field(default=True, description="Include refinement suggestions")
    user_session_id: Optional[str] = Field(None, description="User session ID")
...
class EnhancedSearchResponse(BaseModel):
    ...
    refinement_suggestions: Optional[List[Dict]] = Field(None, description="Refinement suggestions")
    search_metadata: Dict = Field(..., description="Search metadata and analytics")
...
async def get_query_suggestions(
    ...
    analysis_service: QueryAnalysisService = Depends(get_query_analysis_service),
):
    """
    ...
    """
    try:
        logger.info(f"Generating suggestions for query: '{request.original_query}'")
        ...
        similar_queries = analysis_service.find_similar_successful_queries(request.original_query)
        alternative_queries = [sq.query_text for sq in similar_queries]

        # Generate explanation
        issues_found = len(analysis.potential_issues)
        explanation = f"Analyzed query and found {issues_found} potential issues. "
        explanation += f"Generated {len(suggestions)} suggestions to improve results."

        # Prepare analysis metadata
        metadata = {
            "complexity_score": analysis.complexity_score,
            "entities_found": len(analysis.entities_found),
            "issues_identified": [issue.value for issue in analysis.potential_issues],
...
@refinement_router.get("/similar-queries", response_model=SimilarQueriesResponse)
async def get_similar_queries(
    ...
    try:
        ...
        similar_queries = analysis_service.find_similar_successful_queries(query, limit)
...
async def submit_query_feedback(request: QueryFeedbackRequest):
    ...
    """
    try:
        logger.info(f"Receiving feedback for suggestion: '{request.suggested_query}'")
        ...
        response = QueryFeedbackResponse(feedback_id=feedback_id, status="received", message=message)
...
@refinement_router.post("/search/enhanced", response_model=EnhancedSearchResponse)
async def enhanced_search(
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Main search pipeline failed: {search_error}, using empty results")
            search_results = []
            result_count = 0
        ...
        if request.include_suggestions and result_count < 10:
            try:
                ...
                response_data["search_metadata"]["suggestions_generated"] = len(suggestions)
