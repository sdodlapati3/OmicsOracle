...
async def create_batch_job(request: BatchJobRequest):
    ...
    try:
        if not request.queries:
            raise HTTPException(status_code=400, detail="At least one query must be provided")
        ...
        logger.info(f"Created batch job {job_id} with {len(request.queries)} queries")
    ...
    try:
        ...
        async def process_job():
            try:
                ...
                logger.error(f"Background batch processing failed for {job_id}: {e}")
...
async def get_batch_job_results(job_id: str, format: str = "json"):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get batch job results: {str(e)}")
...
async def cleanup_batch_jobs(older_than_hours: int = 24):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to clean up batch jobs: {str(e)}")
