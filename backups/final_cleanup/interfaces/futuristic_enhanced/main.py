...
try:
    ...
    try:
        # Import patch module but we don't need to use it directly
        # The import itself applies the patch
...
try:
    ...
    # Create a fallback AI summary manager that returns None (honest failure)
    class FallbackAISummaryManager:
        ...
        def get_summary_service_status(self):
            return {"ai_service_available": False, "service_type": "Fallback", "initialized": False}

        def get_error_message(self, context: str = "AI summary") -> str:
            return f"{context} unavailable (OmicsOracle services not loaded)"

    ai_summary_manager = FallbackAISummaryManager()
...
class SearchRequest(BaseModel):
    ...
    disable_cache: bool = Field(True, description="Cache disabled by default for fresh results")
    timestamp: Optional[float] = Field(None, description="Client timestamp for cache-busting")
...
class BackendClient:
    ...
    async def search_enhanced(self, request: EnhancedSearchRequest) -> dict:
        ...
        try:
            response = await self.session.post("/api/v2/search/enhanced", json=request.dict(), timeout=30.0)
...
async def process_search_query(query: str, max_results: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            from src.omics_oracle.search.enhanced_query_handler import perform_multi_strategy_search
            ...
            if geo_ids:
                ...
                if search_strategy == "alternative":
                    ...
                    components_found = [k for k, v in components.items() if v and k != "original_query"]
                    if components_found:
                        await log_to_frontend(
                ...
                query_result = await app.state.pipeline.process_query(query, max_results=actual_max_results)

        except ImportError:
            # If enhanced query handler is not available, use the standard pipeline
            await log_to_frontend("[SEARCH] Using standard pipeline search...", "info")
            # Use the pipeline's process_query method (it's async!)
            query_result = await app.state.pipeline.process_query(query, max_results=actual_max_results)
        ...
        if query_result.geo_ids:
            for i, geo_id in enumerate(query_result.geo_ids[:max_results]):
                ...
                # Get AI summary using centralized manager (single source of truth)
                ai_summary = None
                ...
                # If no AI summary from pipeline, generate using centralized manager
                if not ai_summary:
                ...
                dataset_info = {
                    ...
                    "summary": metadata.get("summary"),  # Original GEO summary
                    "ai_summary": ai_summary,  # AI-generated summary (None if unavailable)
        ...
        # Generate search overview summary using centralized manager (single pattern)
        search_overview_summary = ai_summary_manager.generate_ai_summary(
            f"search_overview_{query}",
            {
                "query": query,
                "total_datasets": len(datasets),
                "datasets_with_metadata": len([d for d in datasets if d.get("title") is not None]),
                "datasets_with_summaries": len([d for d in datasets if d.get("summary") is not None]),
                "intent": getattr(query_result, "intent", None),
                "duration": getattr(query_result, "duration", None),
            },
            f"search_{len(datasets)}_results",
        )

        # Use simple fallback message if AI unavailable (no manual building)
        search_summary = (
            search_overview_summary or f"Found {len(datasets)} biomedical datasets for '{query}'."
        )
...
async def health_check():
    ...
    if pipeline is not None:
        ...
        pipeline_info = {
            ...
            "cache_disabled": True,  # Cache permanently disabled for fresh results
            "ai_summary_manager_available": ai_summary_manager.is_ai_service_available(),
        }
        ...
        pipeline_info["critical_components_ready"] = all(
            [
                ...
                pipeline_info.get("ai_summary_manager_available", False),
            ]
        )

    # Include AI summary service status
    ai_service_status = ai_summary_manager.get_summary_service_status()
