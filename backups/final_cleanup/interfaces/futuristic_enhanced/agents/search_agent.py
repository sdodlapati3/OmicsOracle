...
class SearchCache:
    """DEPRECATED: Cache for debugging/analysis only - NOT used for serving results"""

    def __init__(self, ttl_minutes: int = 30):
        self.cache = {}  # Only for debugging/query flow analysis
        self.ttl = timedelta(minutes=ttl_minutes)

    def get(self, query_hash: str) -> Optional[Dict]:
        """Get cached result - DISABLED for user-facing results"""
        # Cache disabled for user-facing results to ensure freshness
        return None

    def set(self, query_hash: str, result: Dict):
        """Cache search result - FOR DEBUGGING ONLY"""
        # Store for debugging/analysis purposes only, not used for serving results
...
class EnhancedSearchAgent(BaseAgent):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.error(f"[ERROR] Failed to initialize enhanced search agent {self.agent_id}: {e}")
            return False
    ...
    async def _handle_search_request(self, message: AgentMessage) -> AgentMessage:
        ...
        self.current_job = message.payload.get("job_id", f"search_{len(self.search_history)}")
        start_time = datetime.utcnow()

        await self._send_update(f"Starting {search_type} search for: {query[:50]}...")

        try:
            # Check cache first
            # CACHE REMOVED: Always perform fresh searches for accurate results
            query_hash = hash(f"{query}_{search_type}_{json.dumps(filters, sort_keys=True)}")
            ...
            logger.info(f"Generated fresh search result for query_hash: {query_hash}")
            ...
            await self._send_update(f"Found {len(results)} results in {processing_time:.2f}s")
    ...
    async def _intelligent_search(self, query: str, filters: Dict) -> List[SearchResult]:
    ...
    async def _basic_search(self, query: str, filters: Dict) -> List[SearchResult]:
            ...
            if result and result.metadata:
                for i, item in enumerate(result.metadata[:50]):  # Limit results
                    enhanced_result = SearchResult(
                        ...
                        publication_date=self._parse_date(item.get("submission_date")),
                        source=item.get("source", "GEO"),
                        confidence_score=self._calculate_confidence(item, search_type),
    ...
    async def _handle_clear_cache(self, message: AgentMessage) -> AgentMessage:
        """Handle cache clearing request - FOR DEBUGGING ONLY (cache no longer used for results)"""
        try:
            cache_size = len(self.search_cache.cache)
            # Clear any remaining cache for debugging analysis only
            ...
            return AgentMessage(
                ...
                payload={
                    ...
                    "message": f"Cleared {cache_size} debugging cache entries (cache no longer affects results)",
                },
                timestamp=datetime.utcnow(),
            )
        except Exception as e:
