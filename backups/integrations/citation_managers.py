...
class CitationManagerIntegration:
    ...
    def format_geo_reference(
        ...
        if isinstance(submission_date, str):
            ...
            formatted_date = str(submission_date) if submission_date else "Unknown"
        ...
        reference = {
            ...
            "issued": {"date-parts": [[formatted_date]] if formatted_date != "Unknown" else []},
    ...
    def to_ris(self, reference: Dict[str, Any]) -> str:
        ...
        if reference.get("abstract"):
            ...
            if len(abstract) > 200:
                # Split into chunks
                chunks = [abstract[i : i + 200] for i in range(0, len(abstract), 200)]
                for chunk in chunks:
        ...
        if reference.get("related_papers"):
            ...
            for paper in reference["related_papers"][:3]:  # Limit to 3
                if paper.get("title") and paper.get("pmid"):
                    ris_lines.append(f'N1  - PMID:{paper["pmid"]} - {paper["title"]}')
    ...
    def to_endnote_xml(self, reference: Dict[str, Any]) -> str:
        ...
        xml_lines.append('<ref-type name="Dataset">59</ref-type>')  # Dataset type
        ...
        if reference.get("title"):
            xml_lines.append(f'<title>{self._escape_xml(reference["title"])}</title>')
        xml_lines.append("</titles>")
        ...
        # Add publisher
        if reference.get("publisher"):
            xml_lines.append(f'<publisher>{self._escape_xml(reference["publisher"])}</publisher>')

        # Add URLs
        xml_lines.append("<urls>")
        if reference.get("URL"):
            xml_lines.append(f'<related-urls><url>{reference["URL"]}</url></related-urls>')
        xml_lines.append("</urls>")

        # Add abstract
        if reference.get("abstract"):
            xml_lines.append(f'<abstract><p>{self._escape_xml(reference["abstract"])}</p></abstract>')

        # Add notes
        if reference.get("note"):
            xml_lines.append(f'<notes><note><p>{self._escape_xml(reference["note"])}</p></note></notes>')
...
def export_geo_references(
    ...
    return citation_manager.export_references(geo_datasets, format_type, include_papers)
