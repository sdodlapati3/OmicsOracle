...
def setup_websockets(app: FastAPI) -> None:
    ...
    async def enhanced_websocket_realtime(
        ...
        try:
            ...
            logger.info(f"Enhanced WebSocket client {connection_id} joined room {room_id}")
            ...
            logger.info(f"Enhanced WebSocket client disconnected from room {room_id}")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    await websocket.send_json({"type": "heartbeat", "client_id": client_id})
