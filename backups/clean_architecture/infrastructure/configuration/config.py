...
class DatabaseConfig:
    ...
    def __post_init__(self):
        ...
        if self.pool_size <= 0:
            raise ConfigurationError("database.pool_size", "Pool size must be positive")
...
class GEOConfig:
    ...
    # SSL configuration
    verify_ssl: bool = field(default_factory=lambda: os.getenv("NCBI_VERIFY_SSL", "true").lower() != "false")
...
class RedisConfig:
    ...
    def __post_init__(self):
        ...
        if self.port <= 0 or self.port > 65535:
            raise ConfigurationError("redis.port", "Port must be between 1 and 65535")
...
class SecurityConfig:
    ...
    def __post_init__(self):
        ...
        if not self.secret_key:
            ...
            self.secret_key = "insecure-default-key-change-in-production"  # nosec B105
...
class AppConfig:
    ...
    def __post_init__(self):
        ...
        if self.port <= 0 or self.port > 65535:
            raise ConfigurationError("app.port", "Port must be between 1 and 65535")

        # Validate worker count
        if self.workers <= 0:
            raise ConfigurationError("app.workers", "Worker count must be positive")
    ...
    def from_env(cls, env_file: Optional[str] = None) -> "AppConfig":
        ...
        config = cls(
            ...
            enable_ai_summarization=os.getenv("ENABLE_AI_SUMMARIZATION", "true").lower() == "true",
        )
