...
from ...shared.exceptions.domain_exceptions import InfrastructureError, ValidationError
...
class GEOSearchRepository(SimpleSearchRepository):
    ...
    async def search(self, query: SearchQuery) -> List[Dataset]:
        ...
        try:
            ...
            for geo_id in geo_ids[: query.max_results]:
                try:
                    # Create a basic dataset from the ID with minimal real data
                    # No fake summaries - let the AI summary manager handle real AI content

                    dataset = Dataset(
                        geo_id=f"GDS{geo_id}",
                        title=f"Dataset {geo_id}",  # Minimal real title
                        summary=None,  # No fake summary - AI manager will provide real one if available
                        description=None,  # No fake description
                        organism=None,  # Will be populated with real data when available
                        platform=None,  # Will be populated with real data when available
                        samples_count=None,  # Will be populated with real data when available
                        metadata={
                            "search_query": query.query_text,
                            "search_source": search_results.get("search_metadata", {}).get(
                                "source", "NCBI_GEO"
                            ),
                            "publication_date": None,  # Will be populated with real data
                    ...
                    logger.warning(f"Failed to create dataset for ID {geo_id}: {e}")
                    continue
    ...
    async def get_by_geo_id(self, geo_id: str) -> Optional[Dataset]:
        ...
        try:
            ...
            raise InfrastructureError(f"Failed to retrieve dataset {geo_id}: {str(e)}") from e
    ...
    async def get_similar(self, dataset: Dataset, limit: int = 10) -> List[Dataset]:
        ...
        try:
            ...
            filtered_datasets = [d for d in similar_datasets if d.geo_id != dataset.geo_id]
            ...
            raise InfrastructureError(f"Failed to find similar datasets: {str(e)}") from e
    ...
    def _map_to_dataset(self, raw_result: dict) -> Dataset:
        ...
        try:
            return Dataset(
                ...
                or self._safe_int(raw_result.get("sample_count")),
                metadata={
    ...
    def _build_similarity_query(self, dataset: Dataset) -> str:
        ...
        if dataset.title:
            ...
            key_terms = [
                ...
                if len(word) > 3 and word not in {"with", "from", "study", "analysis"}
            ]
            if key_terms:
