...
async def create_container() -> Container:
    ...
    await container.register_singleton(MemoryCache, MemoryCache(default_ttl=300))
    ...
    await container.register_factory(SimpleSearchRepository, create_search_repository)
    ...
    await container.register_factory(SearchDatasetsUseCase, create_search_use_case)
    ...
    await container.register_factory(EnhancedSearchDatasetsUseCase, create_enhanced_search_use_case)
...
async def setup_event_subscribers(container: Container) -> None:
    ...
    from ..messaging.search_events import SearchCompletedEvent, SearchFailedEvent, SearchStartedEvent
    ...
    async def handle_search_completed(event: SearchCompletedEvent):
        ...
        logger.info(f"Search completed: {event.query} ({event.results_count} results)")
        # Broadcast to WebSocket clients
        await websocket_service.broadcast(
            {
