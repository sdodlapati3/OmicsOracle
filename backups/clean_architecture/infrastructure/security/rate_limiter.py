...
class TokenBucketBackend(RateLimitBackend):
        ...
        async with self._lock:
            ...
            if bucket["tokens"] >= 1:
                ...
                return RateLimitResult(
                    ...
                    reset_time=now + (config.burst_size - bucket["tokens"]) / config.requests_per_second,
                    metadata={
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            total_requests = sum(bucket["requests"] for bucket in self._buckets.values())
        ...
        async with self._lock:
            ...
            if requests_in_window < max_requests:
                ...
                return RateLimitResult(
                    ...
                    reset_time=window[0] + config.window_size_seconds if window else now,
                    metadata={
...
class FixedWindowBackend(RateLimitBackend):
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            total_windows = len(self._windows)
            total_requests = sum(window["requests"] for window in self._windows.values())
...
class RateLimiter:
    ...
    async def check_rate_limit(self, key: str, config: Optional[RateLimitConfig] = None) -> RateLimitResult:
        """Check if request should be rate limited."""
        effective_config = config or self._per_key_configs.get(key) or self.default_config
...
class RateLimitMiddleware:
    ...
    async def __call__(self, request, call_next):
        ...
        key = self.rate_limiter.get_key_for_request(client_ip, user_id, endpoint)
