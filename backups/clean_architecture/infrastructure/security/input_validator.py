...
class StringLengthRule(ValidationRule):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if length < self.min_length:
            errors.append(f"String too short (minimum {self.min_length} characters)")

        if self.max_length and length > self.max_length:
            errors.append(f"String too long (maximum {self.max_length} characters)")

        is_valid = len(errors) == 0
        return ValidationResult(is_valid, value, errors, value if is_valid else None)
...
def create_string_validator(min_length: int = 0, max_length: Optional[int] = None) -> InputValidator:
