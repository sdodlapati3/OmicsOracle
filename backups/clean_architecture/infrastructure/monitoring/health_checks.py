...
class HealthCheckManager:
    ...
    def start_background_monitoring(self) -> None:
        ...
        for name, interval in self._check_intervals.items():
            if name not in self._background_tasks:
                task = asyncio.create_task(self._background_health_check(name, interval))
                self._background_tasks[name] = task
                logger.info(f"Started background monitoring for {name} (interval: {interval}s)")
    ...
    async def _background_health_check(self, check_name: str, interval: float) -> None:
        """Background task for periodic health checks."""
        while True:
            try:
                await asyncio.sleep(interval)
                result = await self.check_health(check_name)
                logger.debug(f"Background health check {check_name}: {result[check_name].status.value}")
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Background health check {check_name} failed: {e}")
                await asyncio.sleep(min(interval, 60))  # Don't retry too quickly
