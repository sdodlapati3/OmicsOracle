...
class RetryManager:
    ...
    def _should_retry(self, exception: Exception, attempt: int) -> bool:
        ...
        return all(condition.should_retry(exception, attempt) for condition in self._conditions)
        ...
        while attempt <= self.config.max_retries:
            try:
                ...
                if attempt <= self.config.max_retries:
                    ...
                    last_delay = delay
                    logger.warning(f"Retry attempt {attempt} after {delay:.2f}s delay. Error: {e}")
    ...
    def __call__(self, func: Callable) -> Callable:
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                result = await self.retry_manager.retry_async(func, *args, **kwargs)
                if result.success:
                    return result.value
                else:
                    raise result.error
