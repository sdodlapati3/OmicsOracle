...
class FailoverManager:
    ...
    def register_service(self, service_name: str, endpoints: List[ServiceEndpoint]) -> None:
        """Register service endpoints."""
        self._endpoints[service_name] = endpoints
        self._current_indices[service_name] = 0
        logger.info(f"Registered {len(endpoints)} endpoints for service: {service_name}")
    ...
    def get_healthy_endpoint(self, service_name: str) -> Optional[ServiceEndpoint]:
        ...
        if self.config.strategy == FailoverStrategy.PRIORITY:
            ...
            self._current_indices[service_name] = (current_index + 1) % len(healthy_endpoints)
            return endpoint
        else:
            return healthy_endpoints[0]

    def mark_endpoint_unhealthy(self, service_name: str, endpoint_name: str) -> None:
            ...
            for endpoint in self._endpoints[service_name]:
                if endpoint.name == endpoint_name:
                    ...
                    if endpoint.failure_count >= self.config.failure_threshold:
                        ...
                        logger.warning(f"Marked endpoint {endpoint_name} as unhealthy")
                    break
