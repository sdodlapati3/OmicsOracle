...
class ServiceRegistry:
    ...
    def __init__(self, heartbeat_timeout: int = 30):
        ...
        self._services_by_name: Dict[str, List[str]] = {}  # service_name -> [service_ids]
        self._services_by_type: Dict[ServiceType, List[str]] = {}  # service_type -> [service_ids]
    ...
    async def register_service(self, service_info: ServiceInfo) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Service registered: {service_info.service_name} ({service_id})")
            return True
    ...
    async def deregister_service(self, service_id: str) -> bool:
        ...
        try:
            ...
            if service_info.service_name in self._services_by_name:
                self._services_by_name[service_info.service_name].remove(service_id)
            ...
            if service_info.service_type in self._services_by_type:
                self._services_by_type[service_info.service_type].remove(service_id)
            ...
            self.logger.info(f"Service deregistered: {service_info.service_name} ({service_id})")
            return True
        ...
        if service_id in self._services:
            ...
            if old_status != status:
                for callback in self._health_change_callbacks:
                    try:
                        await callback(self._services[service_id], old_status, status)
                    except Exception as e:
        ...
        while self._running:
            try:
                ...
                for service_id in stale_services:
                    service_info = self._services[service_id]
                    if (current_time - service_info.last_heartbeat) > very_stale_timeout:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for service_type in ServiceType:
            services = self.get_services_by_type(service_type)
            healthy_services = [s for s in services if s.is_healthy(self.heartbeat_timeout)]
        ...
        return {
            ...
            "healthy_services": len(
                [s for s in self._services.values() if s.is_healthy(self.heartbeat_timeout)]
            ),
...
class LoadBalancer:
    ...
    def select_service(self, service_name: str, strategy: str = "round_robin") -> Optional[ServiceInfo]:
        """
        Select a service instance using load balancing strategy
        """
        healthy_services = self.registry.get_healthy_services(service_name=service_name)
...
class ServiceClient:
    ...
    async def call_service(
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                service = self.load_balancer.select_service(service_name, load_balance_strategy)

                if not service:
                    self.logger.error(f"No healthy instances of service '{service_name}' found")
                    return None
                ...
                self.logger.error(f"Service call attempt {attempt + 1} failed: {e}")
    ...
    async def get(self, service_name: str, path: str, **kwargs) -> Optional[Dict]:
        """GET request to service"""
        return await self.call_service(service_name, "GET", path, **kwargs)

    async def post(self, service_name: str, path: str, data: Dict, **kwargs) -> Optional[Dict]:
        """POST request to service"""
        return await self.call_service(service_name, "POST", path, data=data, **kwargs)

    async def put(self, service_name: str, path: str, data: Dict, **kwargs) -> Optional[Dict]:
        """PUT request to service"""
        return await self.call_service(service_name, "PUT", path, data=data, **kwargs)

    async def delete(self, service_name: str, path: str, **kwargs) -> Optional[Dict]:
