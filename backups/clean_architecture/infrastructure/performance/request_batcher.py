...
class RequestBatcher(Generic[T, R]):
    ...
    def __init__(
        ...
        # Rate limiting
        self._rate_limiter = TokenBucketRateLimiter(self.config.max_requests_per_second)
        ...
        async with self._processing_lock:
            ...
            if batch_size >= self.config.max_batch_size:
                ...
                timer = asyncio.get_event_loop().call_later(
                    ...
                    lambda: asyncio.create_task(self._process_batch_by_timer(batch_key)),
                )
    ...
    async def _safe_process_batch(self, batch_data: List[T]) -> List[R]:
        ...
        try:
            if asyncio.iscoroutinefunction(self.batch_processor):
                ...
                return await loop.run_in_executor(None, self.batch_processor, batch_data)
...
class BatchStats:
    ...
    def average_batch_processing_time(self) -> float:
        ...
        return self.total_batch_processing_time / self.total_batches if self.total_batches > 0 else 0.0
