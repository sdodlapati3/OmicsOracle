...
class RoomManager:
    ...
    async def join_room(self, connection_id: str, room_id: str, user_id: Optional[str] = None) -> bool:
        """
        Join a connection to a room
        """
        if room_id not in self._rooms:
            self.logger.warning(f"Attempted to join non-existent room: {room_id}")
            return False
    ...
    async def leave_room(self, connection_id: str, room_id: str, user_id: Optional[str] = None) -> bool:
        """
    ...
    async def send_to_room(
        ...
        if room_info.persist_messages:
            ...
            # Limit history size
            if len(self._message_history[room_id]) > room_info.max_message_history:
                self._message_history[room_id] = self._message_history[room_id][
                    -room_info.max_message_history :
                ]

        # Send to connection manager
        await connection_manager.send_to_room(room_id, message, exclude_connections)
    ...
    async def broadcast_to_type(
        ...
        for room_id, room_info in self._rooms.items():
            if room_info.room_type == room_type and room_id not in exclude_rooms:
    ...
    async def get_or_create_search_room(self, search_id: str, user_id: Optional[str] = None) -> str:
        """
            ...
            try:
                ...
                for room_id, room_info in self._rooms.items():
                    if (
                        room_info.auto_cleanup
                        and room_info.connection_count == 0
                        and (current_time - room_info.last_activity) > room_info.cleanup_timeout
                    ):
    ...
    def list_rooms(self, room_type: Optional[RoomType] = None) -> List[RoomInfo]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "total_connections_in_rooms": sum(room.connection_count for room in self._rooms.values()),
            "average_room_size": sum(room.connection_count for room in self._rooms.values())
