...
from .connection_manager import MessageType, WebSocketMessage, connection_manager
...
class RealtimeService:
    ...
    async def start(self):
        ...
        connection_manager.add_disconnection_callback(self._handle_connection_disconnect)
    ...
    async def start_search(
        ...
        room_id = await room_manager.get_or_create_search_room(search_id, user_id)
        ...
        # Update metrics
        self._performance_metrics["active_searches"] = len(self._search_sessions)
    ...
    async def join_search(self, search_id: str, connection_id: str) -> bool:
        ...
        # Join room
        success = await room_manager.join_room(connection_id, search_session.room_id)
        ...
        if success:
            ...
            self.logger.info(f"Connection {connection_id} joined search {search_id}")
    ...
    async def leave_search(self, search_id: str, connection_id: str) -> bool:
        ...
        # Leave room
        success = await room_manager.leave_room(connection_id, search_session.room_id)

        if success:
            search_session.connections.discard(connection_id)
            self.logger.info(f"Connection {connection_id} left search {search_id}")
    ...
    async def update_search_progress(
        ...
        **metadata,
        ...
        if progress.total_steps > 0 and progress_percent is None:
            progress.progress_percent = (progress.completed_steps / progress.total_steps) * 100
        ...
        self.logger.debug(f"Search progress updated: {search_id} - {progress.progress_percent:.1f}%")
    ...
    async def complete_search(
        ...
        # Update metrics
        duration = time.time() - search_session.progress.started_at
        self._update_completion_metrics(duration, len(search_session.progress.errors) == 0)

        # Schedule session cleanup
        asyncio.create_task(self._cleanup_search_session(search_id, delay=300))  # 5 minutes

        self.logger.info(f"Search completed: {search_id} - {search_session.progress.results_found} results")

    async def cancel_search(self, search_id: str, reason: str = "User cancelled"):
        """
    ...
    async def _send_search_event(
        ...
        # Send to search room
        await message_queue.enqueue_to_room(search_session.room_id, message, priority=priority)
    ...
    async def _cleanup_search_session(self, search_id: str, delay: int = 0):
        ...
        if search_session.user_id and search_session.user_id in self._user_searches:
        ...
        if room_info and room_info.auto_cleanup:
            await room_manager.destroy_room(search_session.room_id, "Search session ended")
    ...
    async def _handle_connection_disconnect(self, connection_id: str, connection_info, reason: str):
        """Handle connection disconnection"""
        # Remove connection from all search sessions
        for search_session in self._search_sessions.values():
            search_session.connections.discard(connection_id)

    async def _handle_user_left_room(self, connection_id: str, room_id: str, user_id: Optional[str]):
    ...
    def _update_completion_metrics(self, duration: float, success: bool):
        ...
        if total_searches > 1:
            new_avg = ((current_avg * (total_searches - 1)) + duration) / total_searches
        else:
            new_avg = duration
    ...
    def subscribe_to_event(self, event_type: SearchEventType, callback: Callable):
    ...
    def add_search_callback(self, search_id: str, callback_type: str, callback: Callable):
    ...
    def get_active_searches(self) -> List[SearchSession]:
        ...
        return [session for session in self._search_sessions.values() if session.is_active]
    ...
    def get_search_statistics(self) -> Dict[str, Any]:
        ...
        active_searches = len([s for s in self._search_sessions.values() if s.is_active])
        ...
        return {
            ...
            "total_connections": sum(len(s.connections) for s in self._search_sessions.values()),
