...
class WebSocketService:
        ...
        if failed_connections:
            async with self._lock:
                for connection_id in failed_connections:
                    if connection_id in self._connections:
                        del self._connections[connection_id]
                        logger.warning(f"Removed failed connection: {connection_id}")

        logger.debug(f"Broadcast sent to {success_count} connections")
        return success_count

    async def send_search_progress(self, connection_id: str, progress: Dict[str, Any]) -> bool:
    ...
    async def send_search_results(self, connection_id: str, results: List[Dict[str, Any]]) -> bool:
    ...
    async def send_error(self, connection_id: str, error: str, error_type: str = "general") -> bool:
