...
async def search_datasets_v1(
    ...
    try:
        ...
        search_request = SearchRequestDTO(
            ...
            filters={"organism": organism, "study_type": study_type} if organism or study_type else None,
        )

        # Execute search
        response = await use_case.execute(search_request)

        logger.info(f"v1 search completed: {len(response.datasets)} results for query '{query}'")
        return response
