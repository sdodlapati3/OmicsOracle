...
class SearchDatasetsUseCase:
    ...
    async def execute(self, request: SearchRequestDTO) -> SearchResponseDTO:
        ...
        try:
            ...
            response = SearchResponseDTO.from_search_result(search_result, request)
            ...
            logger.warning(
                f"Search validation failed - " f"processing_id: {processing_id}, " f"error: {e.message}"
            )
            return self._create_error_response(request, processing_id, str(e), start_time)

        except SearchTimeoutError as e:
            logger.warning(
                f"Search timed out - " f"processing_id: {processing_id}, " f"timeout: {e.timeout_seconds}s"
            )
            return self._create_error_response(request, processing_id, str(e), start_time)
    ...
    def _convert_to_domain_query(self, request: SearchRequestDTO) -> SearchQuery:
        """
    ...
    async def execute(self, geo_id: str) -> Optional[SearchResponseDTO]:
        ...
        try:
            ...
            if not geo_id.startswith(("GSE", "GDS", "GPL", "GSM")):
                raise ValidationError(f"Invalid GEO ID format: {geo_id}", field="geo_id")

            # Retrieve dataset
            dataset = await self._search_repository.get_by_geo_id(geo_id.strip())
