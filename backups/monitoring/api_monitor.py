...
class APIMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app: FastAPI, log_to_file: bool = True, log_dir: str = "logs", exclude_paths: List[str] = None
    ):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            log_entry = {**request_info, "response": response_info, "error": status_code >= 400}
    ...
    def update_stats_file(self) -> None:
        ...
        avg_response_time = sum(stat["total_time"] for stat in self.endpoint_stats.values()) / max(
            1, self.request_count
        )
        ...
        stats = {
            ...
            "endpoints": {
                path: {**stat, "avg_time": stat["total_time"] / stat["count"] if stat["count"] > 0 else 0}
...
def setup_api_monitoring(app: FastAPI, log_dir: str = "logs") -> None:
    ...
    app.add_middleware(APIMonitoringMiddleware, log_to_file=True, log_dir=log_dir)
