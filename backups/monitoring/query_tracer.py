...
class QueryTracer:
    ...
    def __init__(
        ...
        logger.info(f"[TRACE] Started query tracing for '{original_query}' with ID: {self.query_id}")
    ...
    def record_step(
        ...
        inputs_safe = self._make_serializable(inputs) if inputs is not None else None
        outputs_safe = self._make_serializable(outputs) if outputs is not None else None
        ...
        if duration is not None:
            ...
            self.trace_data["component_times"][component]["total_time"] += duration
        ...
        # Log the step
        log_level = (
            logging.INFO if status == "success" else logging.WARNING if status == "warning" else logging.ERROR
        )
        logger.log(
            log_level,
    ...
    def record_warning(self, stage: str, message: str, details: Optional[Dict[str, Any]] = None) -> None:
    ...
    def finalize(self, final_result: Optional[Any] = None, status: str = "completed") -> None:
        """
        ...
        if final_result is not None:
            self.trace_data["final_result"] = self._make_serializable(final_result)
    ...
    def generate_report(self) -> str:
        ...
        with open(report_file, "w") as f:
            ...
            f.write(f"- **Duration**: {self.trace_data['duration']:.3f} seconds\n")
            ...
            if self.trace_data["transformations"]:
                f.write("\n## Query Transformations\n\n")
                for i, transform in enumerate(self.trace_data["transformations"], 1):
                    f.write(f"### {i}. {transform['stage']} - {transform['transform_type']}\n\n")
            ...
            f.write("\n## Component Performance\n\n")
            f.write("| Component | Calls | Total Time (s) | Avg Time (s) | Max Time (s) |\n")
            f.write("|-----------|-------|---------------|--------------|-------------|\n")
            for component, times in self.trace_data["component_times"].items():
                avg_time = times["total_time"] / times["calls"] if times["calls"] > 0 else 0
                f.write(
            ...
            for step in self.trace_data["steps"]:
                status_icon = (
                    "✅" if step["status"] == "success" else "⚠️" if step["status"] == "warning" else "❌"
                )
                f.write(
            ...
            if self.trace_data["warnings"]:
                ...
                for i, warning in enumerate(self.trace_data["warnings"], 1):
                    ...
                    if warning.get("details"):
                        f.write("- **Details**:\n\n")
                        f.write("```\n")
                        f.write(json.dumps(warning["details"], indent=2, default=str))
                        f.write("\n```\n\n")
            ...
            if self.detailed and self.trace_data.get("final_result"):
                f.write("\n## Final Result\n\n")
                f.write("```\n")
                f.write(json.dumps(self.trace_data["final_result"], indent=2, default=str))
                f.write("\n```\n")
    ...
    def trace_context(self, stage: str, component: str):
        ...
        try:
            ...
            self.record_step(stage, component, duration=duration, status=status, error=error)
