...
class Phase6IntegrationTester:
    ...
    async def test_v1_compatibility_api(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                ...
                )  # Either results or handled error
                return data
    ...
    async def test_v2_advanced_api(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                f"{self.base_url}/api/v2/search/advanced", params=params
            ) as response:
                ...
                assert (
                    response.headers.get("X-API-Supported-Versions")
                    == "1.0,2.0"
                )
    ...
    async def test_cache_stats_endpoint(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/v2/cache/stats"
            ) as response:
    ...
    async def test_service_registry_endpoint(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/v2/services/registry"
            ) as response:
                # Allow both success and service unavailable
                assert response.status in [200, 500, 503]
    ...
    async def test_detailed_health_check(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/v2/health/detailed"
            ) as response:
    ...
    async def test_legacy_websocket_compatibility(self):
        ...
        try:
            ...
            async with websockets.connect(uri, timeout=5) as websocket:
                ...
                assert (
                    "upgrade_notice" in welcome_data
                )  # Should suggest v2 upgrade
    ...
    async def run_comprehensive_test_suite(self):
        ...
        await self.run_test(
            "API Version Discovery", self.test_api_version_discovery
        )
        await self.run_test(
            "V1 API Compatibility", self.test_v1_compatibility_api
        )
        await self.run_test("V2 Advanced API", self.test_v2_advanced_api)
        await self.run_test(
            "API Versioning Middleware", self.test_api_versioning_middleware
        )
        ...
        await self.run_test(
            "Service Registry Endpoint", self.test_service_registry_endpoint
        )
        await self.run_test(
