...
class OmicsOracleTestRunner:
    ...
    def discover_tests(self) -> List[TestResult]:
        ...
        discovered_tests.sort(key=lambda x: (
            self.test_categories[x.category]["priority"],
            x.name
        ))
    ...
    def run_all_tests(self, categories: Optional[List[str]] = None) -> None:
        ...
        for test_result in all_tests:
            ...
            print(f"{color}{test_result.status:10}{reset_color} "
    ...
    def print_summary(self) -> None:
        ...
        for category, stats in self.summary['categories'].items():
            if stats['total'] > 0:
                ...
                print(f"{category:12}: {stats['passed']:2}/{stats['total']:2} "
    ...
    def validate_event_flow_coverage(self) -> Dict[str, Any]:
        ...
        server_init_events = [
            ...
            "Entrez Email Patch", "Config Loading", "NCBI Email Config", "Bio.Entrez Setup",
            "Pipeline Initialization", "Component Initialization", "GEO Client Init",
            "NLP Interpreter Init", "BiomedicalNER Init", "Synonym Mapper Init",
            "Summarizer Init", "Search Service Init", "Cache Disabling",
            "Progress Callback Setup", "API Routes Setup", "Static Files Setup",
            "CORS Setup", "WebSocket Manager Setup"
        ]
        
        search_process_events = [
            "Search Request", "Request Validation", "Pipeline Availability Check",
            "WebSocket Notification", "Query Processing", "Entity Extraction",
            "Query Expansion", "Intent Detection", "GEO Database Search",
            "NCBI Connection", "ESearch Request", "ESummary Request",
            "Result Processing", "Metadata Extraction", "Result Filtering",
            "AI Summarization", "OpenAI API Request", "Summary Generation",
            "Result Formatting", "Result Sorting", "Response Preparation",
            "Quality Check", "Response Creation"
        ]
        
        frontend_events = [
            "WebSocket Connection", "WebSocket Connection Acceptance",
            "WebSocket Message Handler", "Progress Updates", "Progress Event Parsing",
            "Progress Bar Update", "Live Monitor Update", "Results Preparation",
            "Results JSON Parsing", "Search History Update", "Results Rendering",
            "Dataset Card Creation", "GEO Summary Display", "AI Summary Display",
            "Error Handling", "API Error Processing", "WebSocket Error Processing",
            "UI Error Display"
        ]
...
def main():
    ...
    parser.add_argument("--report", default="test_execution_report.json",
    ...
    while project_root.parent != project_root:
        if any((project_root / marker).exists() for marker in ["pyproject.toml", "requirements.txt", "setup.py"]):
            break
    ...
    try:
        ...
        if args.validate_coverage:
            ...
            print(f"Event Coverage: {coverage['covered_events']}/{coverage['total_events']} "
