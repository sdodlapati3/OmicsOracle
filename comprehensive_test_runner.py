...
class OmicsOracleTestRunner:
    ...
    def discover_tests(self) -> List[TestResult]:
        ...
        discovered_tests.sort(
            key=lambda x: (self.test_categories[x.category]["priority"], x.name)
        )
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        errors = len(
            [
                ...
                if t.status in ["ERROR", "TIMEOUT", "NOT_FOUND"]
            ]
        )
        ...
        for category in self.test_categories.keys():
            category_tests = [
                t for t in self.test_results if t.category == category
            ]
            category_summary[category] = {
                "total": len(category_tests),
                "passed": len(
                    [t for t in category_tests if t.status == "PASSED"]
                ),
                "failed": len(
                    [t for t in category_tests if t.status == "FAILED"]
                ),
                "errors": len(
                    [
                        t
        ...
        total_duration = sum(
            t.duration for t in self.test_results if t.duration
        )
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"Passed:      {self.summary['passed']} ({self.summary['success_rate']:.1f}%)"
        )
        ...
        if self.summary["failed"] > 0 or self.summary["errors"] > 0:
            ...
            for result in self.summary["detailed_results"]:
                if result["status"] not in ["PASSED"]:
                    print(
                        f"{result['status']:10} {result['category']:12} {result['name']}"
                    )
                    if result["error"]:
    ...
    def validate_event_flow_coverage(self) -> Dict[str, Any]:
        ...
        all_events = (
            server_init_events + search_process_events + frontend_events
        )
        ...
        for event in all_events:
            ...
            for test_result in self.test_results:
                if any(
                    keyword in test_result.name.lower()
                    for keyword in event.lower().split()
                ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OmicsOracle Comprehensive Test Runner"
    )
    parser.add_argument(
        "--categories",
        nargs="+",
    ...
    try:
        ...
        if args.validate_coverage:
            ...
            coverage_path = (
                project_root / "test_reports" / "event_coverage_report.json"
            )
