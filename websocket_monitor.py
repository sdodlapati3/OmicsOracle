...
def log_websocket_message(message: Union[Dict[str, Any], str]):
    ...
    try:
        ...
        if isinstance(message, str):
            ...
            if message.strip().startswith("{"):
                try:
                    ...
                    return log_websocket_message(
                        json_message
                    )  # Recursively process as JSON
        ...
        # Check if it's a search result
            ...
            logger.info(
                f"Search Result: Found {result_count} datasets for query '{message.get('query')}'"
            )
        # Check if it's an error
            ...
            logger.debug(
                f"WebSocket message type: {msg_type} - {message_str[:100]}..."
            )
...
def setup_websocket_monitoring(app):
    ...
    if hasattr(app, "state") and hasattr(app.state, "connection_manager"):
        ...
        if hasattr(connection_manager, "broadcast_progress"):
            ...
            async def broadcast_progress_with_logging(
                query_id, stage, message, percentage, detail=None
            ):
                try:
                    progress_data = {
                        "type": "progress",
                    ...
                    logger.error(
                        f"Error in broadcast_progress_with_logging: {e}"
                    )

                return await original_broadcast_progress(
                    query_id, stage, message, percentage, detail
                )

            # Replace the broadcast_progress method
            connection_manager.broadcast_progress = (
                broadcast_progress_with_logging
            )
    ...
    logger.warning(
        "Could not set up WebSocket monitoring - connection manager not found"
    )
    return False
