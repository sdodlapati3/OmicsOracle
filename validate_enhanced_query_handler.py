...
def test_component_extraction(queries):
    ...
    for query in queries:
        ...
        result = {
            ...
            "component_count": sum(
                1 for v in components.values() if v is not None
            ),
        }
        ...
        if result["component_count"] > 0:
            logger.info(
                f"✅ Successfully extracted {result['component_count']} components"
            )
...
def test_synonym_expansion(queries):
    ...
    for query in queries:
        ...
        if total_expansions > 0:
            logger.info(
                f"✅ Successfully expanded to {total_expansions} synonyms"
            )
...
def test_alternative_query_generation(queries):
    ...
    for query in queries:
        ...
        if alt_queries:
            logger.info(
                f"✅ Successfully generated {len(alt_queries)} alternative queries"
            )
            for i, alt_query in enumerate(
                alt_queries[:5], 1
            ...
            logger.warning(
                f"⚠️ No alternative queries generated for: '{query}'"
            )
...
async def test_full_enhanced_search(queries, max_results=5):
    ...
    for query in queries:
        try:
            ...
            if geo_ids:
                logger.info(
                    f"✅ Found {len(geo_ids)} results using {result['search_strategy']} strategy"
                )
...
def generate_validation_report(
    ...
    with open(report_path, "w") as f:
        ...
        f.write("## Summary\n\n")
        f.write(
            f"- Component Extraction: {len(component_results)} queries tested\n"
        )
        f.write(f"- Synonym Expansion: {len(synonym_results)} queries tested\n")
        f.write(
            f"- Alternative Query Generation: {len(alt_query_results)} queries tested\n"
        )
        f.write(
            f"- Full Search Pipeline: {len(search_results)} queries tested\n\n"
        )
        ...
        for result in synonym_results:
            ...
            for component, values in result["expanded"].items():
                if values:
                    f.write(
                        f"- **{component}** ({len(values)} synonyms): {', '.join(values)}\n"
                    )
            f.write("\n")
        ...
        for result in alt_query_results:
            f.write(f"### Query: '{result['query']}'\n\n")
            f.write(
                f"Generated {result['alternative_count']} alternative queries:\n\n"
            )
            for i, alt_query in enumerate(
                result["alternative_queries"][:10], 1
            ):
                f.write(f"{i}. {alt_query}\n")
            if len(result["alternative_queries"]) > 10:
                f.write(
                    f"... and {len(result['alternative_queries']) - 10} more\n"
                )
            f.write("\n")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate OmicsOracle enhanced query handler"
    )
    parser.add_argument(
        "--queries",
        nargs="+",
    ...
    parser.add_argument(
        "--max-results", type=int, default=5, help="Maximum search results"
    )
    parser.add_argument(
        "--skip-search",
        action="store_true",
    ...
    if not args.skip_search:
        search_results = await test_full_enhanced_search(
            args.queries, args.max_results
        )
    ...
    logger.info(
        f"All validation tests completed. Report available at: {report_path}"
    )
