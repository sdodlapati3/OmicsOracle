...
async def test_direct_ncbi_query():
    ...
    try:
        ...
        handle = Entrez.esearch(
            db="gds", term="methylation[All Fields]", retmax=5
        )
        ...
        if "IdList" in record and record["IdList"]:
            logger.info(
                f"Successfully queried NCBI. Found {len(record['IdList'])} results."
            )
...
async def validate_geo_client():
    ...
    logger.info(
        f"NCBI_EMAIL environment variable: {ncbi_email_env or 'Not set'}"
    )
    ...
    try:
        ...
        if hasattr(geo_client, "_client") and hasattr(
            ...
            logger.warning(
                "GEO client doesn't have expected email attribute structure"
            )
...
async def main():
    ...
    if geo_client_valid and ncbi_query_valid:
        ...
        logger.warning(
            "⚠️ NCBI configuration has issues that need to be resolved"
        )
        ...
        logger.info(
            "2. Restart the application to load the updated configuration"
        )
        logger.info(
            "3. Check if your IP is blocked by NCBI (too many requests)"
        )
        logger.info(
            "4. Consider using an NCBI API key for higher request limits"
        )
