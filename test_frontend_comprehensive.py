...
def test_endpoint(
    url, description, method="GET", data=None, expected_status=200
):
...
def main():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"🔧 Pipeline Available: {health_data.get('pipeline_available', False)}"
            )
        else:
            print(
                f"⚠️  Frontend responding but health check failed: {response.status_code}"
            )
    except Exception as e:
        print(f"❌ Frontend is not responding at {base_url}")
        print(f"   Error: {e}")
        print(
            "Please start the frontend with: ./start.sh --frontend-only --frontend-port 8002"
        )
        sys.exit(1)
        ...
        if result["success"] and method == "POST" and "search" in url:
            ...
            try:
                ...
                if isinstance(resp_data, dict):
                    ...
                    print(
                        f"   📊 Query: '{query[:50]}...' → {total} results in {search_time:.2f}s"
                    )
            except:
                pass
    ...
    search_results = [
        r for r in results if r["success"] and "search" in r["url"].lower()
    ]
    if search_results:
        avg_search_time = sum(r["test_time"] for r in search_results) / len(
            search_results
        )
