                ...
                try:
                    ...
                    try:
                        ...
                        if isinstance(data, dict) and data.get("type") == "progress":
                            ...
                            timestamp = datetime.fromtimestamp(data.get("timestamp", time.time())).strftime(
                                "%H:%M:%S"
                            )
                            ...
                            print(f"[{timestamp}] [{percentage.rjust(6)}] {stage}: {msg}")
...
async def send_search_query(query="dna methylation immune cells", max_results=10):
