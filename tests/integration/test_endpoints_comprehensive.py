...
def validate_enhanced_query_results(result):
    ...
    if "datasets" in response and isinstance(response["datasets"], list):
        if not response["datasets"] and response.get("total_found", 0) > 0:
            validation["issues"].append("Total found > 0 but no datasets returned")
...
def test_enhanced_query_endpoints(base_url):
    ...
    for query in test_queries:
        ...
        result = test_endpoint(url, f"Enhanced Search: '{query}'", expected_status=200)
        results.append(result)

        # Test with trace enabled
        url = f"{base_url}/api/v2/search/enhanced?query={query}&trace=true"
        result = test_endpoint(url, f"Enhanced Search with Trace: '{query}'", expected_status=200)
        results.append(result)

        # Test component extraction
        url = f"{base_url}/api/v2/query/components?query={query}"
        result = test_endpoint(url, f"Query Component Extraction: '{query}'", expected_status=200)
        results.append(result)
...
def main():
    parser = argparse.ArgumentParser(description="Test OmicsOracle API endpoints")
    parser.add_argument(
        "--base-url",
