...
class TestRealTimeMonitoring:
    ...
    def test_monitoring_integration_with_pipeline(self, mock_pipeline):
        ...
        ws_monitor.record_message(client_id, {
            'type': 'query_progress',
            'query_id': query_id,
            'status': 'searching',
            'progress': 0.3,
        ...
        # 4. Query completed
        pipeline_monitor.record_query_completion(query_id, 
                                                  results_count=5, 
                                                  timestamp=time.time())
    ...
    def test_monitoring_error_scenarios(self):
        ...
        error_messages = [msg for msg in ws_monitor.message_history 
    ...
    def test_monitoring_performance_metrics(self):
        ...
        for i in range(5):
            ...
            pipeline_monitor.record_query_start(query_id, f"test query {i}", 
                                               timestamp=start_time + i)
            pipeline_monitor.record_query_completion(query_id, 
                                                    results_count=10 + i, 
                                                    timestamp=start_time + i + 1.5)
