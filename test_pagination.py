            ...
            try:
                async with session.post(f"{base_url}/search", data=form_data) as response:
                    if response.status == 200:
                        ...
                        print(f"   📄 Page: {pagination.get('current_page', 'N/A')} of {pagination.get('total_pages', 'N/A')}")
                        print(f"   🔢 Total: {pagination.get('total_results', 'N/A')} datasets")
                        print(f"   📍 Range: {pagination.get('start_index', 'N/A')}-{pagination.get('end_index', 'N/A')}")
                        print(f"   ⬅️  Has Previous: {pagination.get('has_previous', False)}")
                        print(f"   ➡️  Has Next: {pagination.get('has_next', False)}")
